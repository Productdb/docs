...
const date = require('./date-19f3f07a.js');
...
const dyteClient = require('./dyte-client-b1a68202.js');
...
const DyteChannelHeader = class {
  constructor(hostRef) {
    ...
    this.searchDismissed = index.createEvent(this, "searchDismissed", 7);
  ...
  renderChannelDetails() {
    return (index.h("dyte-dialog", { open: true, onDyteDialogClose: () => {
        this.showChannelDetailsDialog = false;
      }, iconPack: this.iconPack, t: this.t }, index.h("dyte-channel-details", { members: this.members, channel: this.channel })));
  }
  render() {
    if (!this.channel) {
      return null;
    }
    return (index.h(index.Host, null, this.showChannelDetailsDialog && this.renderChannelDetails(), index.h("header", { class: {
        searching: this.showSearchBar,
      } }, this.showBackButton && (index.h("dyte-button", { kind: "icon", variant: "secondary", class: "back-btn", onClick: () => {
        this.back.emit();
      } }, index.h("dyte-icon", { icon: this.iconPack.chevron_left }))), index.h("div", { class: "channel-details" }, index.h("span", { class: "name" }, this.channel.displayName), !this.channel.isDirectMessage && (index.h("span", { class: "members" }, this.members.map((member) => member.name).join(', ')))), index.h("div", { class: "channel-tools" }, !this.channel.isDirectMessage && (index.h("dyte-tooltip", { label: this.t('chat.channel_members'), iconPack: this.iconPack, t: this.t, variant: "primary" }, index.h("dyte-button", { iconPack: this.iconPack, t: this.t, kind: "button", variant: "secondary", size: "md", onClick: () => {
        this.showChannelDetailsDialog = !this.showChannelDetailsDialog;
      }, class: "br-primary-btn" }, index.h("dyte-icon", { icon: this.iconPack.people, iconPack: this.iconPack, t: this.t }))))))));
  }
...
const DyteChannelSelectorView = class {
  constructor(hostRef) {
    index.registerInstance(this, hostRef);
    this.channelChange = index.createEvent(this, "channelChange", 7);
  ...
  render() {
    ...
    return (index.h(index.Host, null, this.viewAs === 'list' && index.h("slot", { name: "header" }), shouldShowDropdown && (index.h("div", { class: {
        dropdown: this.viewAs === 'dropdown',
        scrollbar: this.viewAs === 'dropdown',
        list: this.viewAs === 'list',
      } }, !this.disableSearch && (index.h("div", { class: "search-container", ref: (el) => (this.$searchEl = el) }, index.h("input", { type: "text", placeholder: this.t('chat.search_conversations'), value: this.searchQuery, onInput: (e) => (this.searchQuery = e.target.value) }), index.h("dyte-icon", { icon: this.iconPack.search }))), index.h("div", { class: "channels-container scrollbar", ref: (el) => (this.$listEl = el) }, filteredChannels.map((channel) => {
      return (index.h("button", { class: { channel: true, active: this.selectedChannelId === channel.id }, onClick: () => this.onChannelClickHandler(channel) }, !this.hideAvatar && (index.h("div", null, channel.icon ? (index.h("dyte-icon", { class: "avatar-icon", icon: this.iconPack[channel.icon] })) : (index.h("dyte-avatar", { participant: {
          name: channel.name,
          picture: channel.avatarUrl,
        } })))), index.h("div", { class: "channel-data" }, index.h("div", { class: "col" }, index.h("div", { class: "name" }, channel.name), channel.latestMessage && (index.h("div", { class: {
          'last-message': true,
          'no-message': !channel.latestMessage,
        } }, index.h("dyte-text-message-view", { isMarkdown: true, text: channel.latestMessage })))), index.h("div", { class: "col channel-meta" }, channel.latestMessageTime && (index.h("time", { class: "time" }, this.getTimeLabel(channel.latestMessageTime))), channel.unreadCount > 0 && (index.h("div", { class: "unread-count" }, channel.unreadCount))))));
    })))), this.viewAs === 'dropdown' && (index.h("button", { class: "dropdown-trigger", onClick: this.toggleDropdown }, index.h("span", null, this.selectedChannelId &&
      `${this.t('to')} ${this.getChannelById(this.selectedChannelId).name}`, this.getTotalUnreads() > 0 && (index.h("div", { class: "unread-count" }, this.getTotalUnreads()))), index.h("dyte-icon", { icon: this.showDropdown ? this.iconPack.chevron_up : this.iconPack.chevron_down })))));
  }
...
const DyteChatComposerView = class {
  constructor(hostRef) {
    index.registerInstance(this, hostRef);
    this.onNewMessage = index.createEvent(this, "newMessage", 7);
    this.onEditMessage = index.createEvent(this, "editMessage", 7);
    this.onEditCancel = index.createEvent(this, "editCancel", 7);
    this.onQuotedMessageDismiss = index.createEvent(this, "quotedMessageDismiss", 7);
    this.textMessage = '';
    this.sendFile = () => {
    ...
    this.onTextChangeHandler = (event) => {
      ...
      userPrefs.gracefulStorage.setItem(this.storageKey, event.detail);
    };
    ...
    this.cleanup = () => {
      this.textMessage = '';
      this.fileToUpload = null;
      userPrefs.gracefulStorage.setItem(this.storageKey, '');
  ...
  connectedCallback() {
    this.textMessage = this.message || userPrefs.gracefulStorage.getItem(this.storageKey) || '';
  }
  componentWillUpdate() {
    this.textMessage = this.message || userPrefs.gracefulStorage.getItem(this.storageKey) || '';
  }
  componentDidLoad() {
    if (this.message) {
      index.writeTask(() => this.$textComposer.setText(this.message, true));
    }
  }
  render() {
    const uiProps = { iconPack: this.iconPack, t: this.t };
    return (index.h(index.Host, null, this.canSendTextMessage && this.isEmojiPickerOpen && (index.h("dyte-emoji-picker", Object.assign({ part: "emoji-picker", onPickerClose: () => {
        this.isEmojiPickerOpen = false;
      }, onDyteEmojiClicked: (e) => {
        this.textMessage += e.detail;
        this.$textComposer.setText(this.textMessage, true);
      } }, uiProps))), index.h("slot", { name: "chat-addon" }), this.quotedMessage && this.quotedMessage.length !== 0 && (index.h("div", { class: "quoted-message-container", part: "quoted-message-container" }, index.h("div", { class: "quoted-message scrollbar" }, index.h("dyte-text-message-view", { text: this.quotedMessage, isMarkdown: true })), index.h("div", null, index.h("dyte-icon", Object.assign({ "aria-label": this.t('dismiss'), class: "dismiss", icon: this.iconPack.dismiss, onClick: this.onQuotedMessageDismissHandler }, uiProps))))), index.h("div", { class: "composer-container" }, index.h("div", { class: "composers" }, this.fileToUpload && (index.h("dyte-draft-attachment-view", { attachment: this.fileToUpload, onDeleteAttachment: () => (this.fileToUpload = null) })), !this.fileToUpload && (index.h("dyte-text-composer-view", { value: this.textMessage, placeholder: this.inputTextPlaceholder, onTextChange: this.onTextChangeHandler, keyDownHandler: this.onKeyDownHandler, maxLength: chat.MAX_TEXT_LENGTH, ref: (el) => (this.$textComposer = el) }))), index.h("div", { class: "chat-buttons", part: "chat-buttons" }, index.h("div", { class: "left", part: "chat-buttons-left" }, !this.fileToUpload && !this.isEditing && (index.h("div", null, this.canSendFiles && [
      index.h("dyte-file-picker-button", { onFileChange: (event) => this.onFileUploadHandler('file', event.detail) }),
      index.h("dyte-file-picker-button", { filter: "image/*", label: this.t('chat.send_img'), icon: "image", onFileChange: (event) => this.onFileUploadHandler('image', event.detail) }),
    ], this.canSendTextMessage && !this.disableEmojiPicker && (index.h("dyte-emoji-picker-button", { isActive: this.isEmojiPickerOpen, onClick: () => {
        this.isEmojiPickerOpen = !this.isEmojiPickerOpen;
      } })), index.h("slot", { name: "chat-buttons" })))), index.h("div", { class: "right", part: "chat-buttons-right" }, !this.isEditing && (index.h("dyte-tooltip", Object.assign({ variant: "primary", label: this.t('chat.send_msg'), delay: 2000 }, uiProps), index.h("dyte-button", Object.assign({ kind: "icon", onClick: () => this.handleSendMessage(), title: this.t('chat.send_msg') }, uiProps), index.h("dyte-icon", { icon: this.iconPack.send })))), this.isEditing && (index.h("div", { class: "edit-buttons" }, index.h("dyte-tooltip", Object.assign({ variant: "secondary", label: this.t('cancel'), delay: 2000 }, uiProps), index.h("dyte-button", Object.assign({ kind: "icon", variant: "secondary", onClick: () => this.handleEditCancel(), title: this.t('cancel') }, uiProps), index.h("dyte-icon", { icon: this.iconPack.dismiss }))), index.h("dyte-tooltip", Object.assign({ variant: "primary", label: this.t('chat.update_msg'), delay: 2000 }, uiProps), index.h("dyte-button", Object.assign({ kind: "icon", onClick: () => this.handleEditMessage(), title: this.t('chat.send_msg') }, uiProps), index.h("dyte-icon", { icon: this.iconPack.checkmark }))))))))));
  }
};
...
const DyteChatMessagesUi = class {
  constructor(hostRef) {
    index.registerInstance(this, hostRef);
    this.onPinMessage = index.createEvent(this, "pinMessage", 7);
  ...
  connectedCallback() {
    var _a;
    this.lastReadTimestamp = (_a = userPrefs.chatUnreadTimestamps['everyone']) !== null && _a !== void 0 ? _a : new Date('0001-01-01T00:00:00Z');
        ...
        for (const entry of entries) {
          if (entry.isIntersecting) {
            ...
            if (currTimestamp > this.lastReadTimestamp.getTime()) {
              ...
              userPrefs.chatUnreadTimestamps[this.selectedGroup] = new Date();
            }
          }
        }
      });
  ...
  render() {
    ...
    return (index.h(index.Host, null, index.h("div", { class: "chat-container scrollbar", ref: (el) => (this.$chat = el), part: "container" }, index.h("div", { class: "spacer", part: "spacer" }), index.h("div", { class: "chat", part: "chat" }, (_a = this.messages) === null || _a === void 0 ? void 0 : _a.map((item) => {
      if (item.type === 'chat') {
        const { message } = item;
        ...
        const isContinued = !isUnread &&
          ...
          date.differenceInMinutes(message.time, prevMessage === null || prevMessage === void 0 ? void 0 : prevMessage.time) < 2;
        prevMessage = message;
        return (index.h("div", { "is-continued": isContinued, key: item.message.id, ref: this.observeMessage, "data-timestamp": message.time.getTime(), class: message.pinned ? 'pinned' : '' }, isUnread && (index.h("div", { class: "new-chat-marker", part: "new-chat-marker" }, this.t('chat.new'))), index.h("div", { class: "message-wrapper" }, index.h("dyte-message-view", { time: message.time, actions: this.getMessageActions(message), authorName: message.displayName, hideAvatar: true, hideAuthorName: true, hideMetadata: true, viewType: 'incoming', variant: "plain", onAction: (event) => this.onMessageActionHandler(event.detail, message) }, index.h("div", null, !isContinued && (index.h("div", { class: "head" }, index.h("div", { class: "name" }, message.displayName), !!message.time && (index.h("div", { class: "time", title: date.formatDateTime(message.time) }, date.elapsedDuration(message.time, new Date(Date.now())))))), index.h("div", { class: "body" }, message.type === 'text' && (index.h("dyte-text-message-view", { text: message.message, isMarkdown: true })), message.type === 'file' && (index.h("dyte-file-message-view", { name: message.name, url: message.link, size: message.size })), message.type === 'image' && (index.h("dyte-image-message-view", { url: message.link, onPreview: () => {
            this.stateUpdate.emit({ image: message });
            store.state.image = message;
          } }))))), message.pinned && (index.h("div", { class: "pin-button", part: "pin-button" }, index.h("dyte-tooltip", { label: this.t('unpin'), iconPack: this.iconPack, t: this.t }, index.h("dyte-button", { kind: "icon", variant: "ghost", onClick: () => this.onMessageActionHandler('pin_message', message), iconPack: this.iconPack, t: this.t, disabled: !this.canPinMessages }, index.h("dyte-icon", { icon: this.iconPack.pin, iconPack: this.iconPack, t: this.t, size: "sm" }))))))));
      }
      return null;
    }))), index.h("div", { class: "show-new-messages-ctr" }, index.h("dyte-button", { class: {
        'show-new-messages': true,
        active: this.showLatestMessageButton,
      }, kind: "icon", part: "show-new-messages", onClick: this.scrollToBottom, iconPack: this.iconPack, t: this.t }, index.h("dyte-icon", { icon: this.iconPack.chevron_down, iconPack: this.iconPack, t: this.t })))));
  }
  static get watchers() { return {
...
const DyteChatMessagesUiPaginated = class {
  constructor(hostRef) {
    index.registerInstance(this, hostRef);
    this.editMessageInit = index.createEvent(this, "editMessageInit", 7);
    this.pageSize = 25;
    ...
    this.hasChildren = (message, isContinued) => {
      ...
      return index.h("div", { ref: (el) => el.appendChild(node) });
    };
  ...
  render() {
    return (index.h(index.Host, null, index.h("dyte-paginated-list", { ref: (el) => (this.$paginatedListRef = el), pageSize: this.pageSize, pagesAllowed: 3, fetchData: this.getChatMessages, createNodes: this.createChatNodes, selectedItemId: this.selectedChannelId, emptyListLabel: this.t('chat.empty_channel') }, index.h("slot", null))));
  }
...
const DyteChatSearchResults = class {
  constructor(hostRef) {
    ...
    this.nodeRenderer = (messages) => {
      return messages.map((message) => (index.h("dyte-chat-message", { key: message.id, message: message, disableControls: true })));
    };
    this.meeting = undefined;
  ...
  render() {
    return (index.h(index.Host, null, index.h("dyte-paginated-list", { pageSize: this.pageSize, pagesAllowed: 3, fetchData: this.searchMessages, createNodes: this.nodeRenderer, selectedItemId: this.query })));
  }
};
...
const DyteDialogManager = class {
  constructor(hostRef) {
    ...
    this.stageStatusUpdateListener = (status) => {
      var _a;
      if (!((_a = this.states) === null || _a === void 0 ? void 0 : _a.activeJoinStage) && !store.state.activeJoinStage)
        return;
      if (status === 'ON_STAGE')
        this.updateStoreState('activeJoinStage', false);
    };
  ...
  render() {
    ...
    if ((states === null || states === void 0 ? void 0 : states.image) != null) {
      ...
      return (index.h(index.Host, null, index.h("dyte-dialog", { open: true, onDyteDialogClose: onImageClose, hideCloseButton: true, iconPack: this.iconPack, t: this.t }, index.h(index$2.Render, { element: "dyte-image-viewer", defaults: defaults, props: { image, onClose: onImageClose } }))));
    }
    else if ((states === null || states === void 0 ? void 0 : states.activeSettings) === true) {
      return (index.h(index.Host, null, index.h("dyte-dialog", { open: true, onDyteDialogClose: () => this.updateStoreState('activeSettings', false), iconPack: this.iconPack, t: this.t }, index.h(index$2.Render, { element: "dyte-settings", defaults: defaults }))));
    }
    else if ((states === null || states === void 0 ? void 0 : states.activeDebugger) === true) {
      return (index.h(index.Host, null, index.h("dyte-dialog", { open: true, onDyteDialogClose: () => this.updateStoreState('activeDebugger', false), iconPack: this.iconPack, t: this.t }, index.h(index$2.Render, { element: "dyte-debugger", defaults: defaults }))));
    }
    else if ((states === null || states === void 0 ? void 0 : states.activeLeaveConfirmation) === true) {
      return (index.h(index.Host, null, index.h("dyte-dialog", { open: true, onDyteDialogClose: () => this.updateStoreState('activeLeaveConfirmation', false), iconPack: this.iconPack, t: this.t }, index.h(index$2.Render, { element: "dyte-leave-meeting", defaults: defaults }))));
    }
    else if (((_a = states === null || states === void 0 ? void 0 : states.activePermissionsMessage) === null || _a === void 0 ? void 0 : _a.enabled) === true) {
      return (index.h(index.Host, null, index.h("dyte-dialog", { open: true, onDyteDialogClose: () => this.updateStoreState('activePermissionsMessage', { enabled: false }), iconPack: this.iconPack, t: this.t }, index.h(index$2.Render, { element: "dyte-permissions-message", defaults: defaults }))));
    }
    else if ((states === null || states === void 0 ? void 0 : states.activeRemoteAccessManager) === true) {
      return (index.h(index.Host, null, index.h("dyte-dialog", { open: true, onDyteDialogClose: () => this.updateStoreState('activeRemoteAccessManager', false), iconPack: this.iconPack, t: this.t }, index.h(index$2.Render, { element: "dyte-remote-access-manager", defaults: defaults }))));
    }
    else if (((_b = states === null || states === void 0 ? void 0 : states.activeBreakoutRoomsManager) === null || _b === void 0 ? void 0 : _b.active) === true) {
      return (index.h(index.Host, null, index.h("dyte-dialog", { open: true, onDyteDialogClose: () => this.updateStoreState('activeBreakoutRoomsManager', {
          active: false,
          data: undefined,
        }), iconPack: this.iconPack, t: this.t }, index.h(index$2.Render, { element: "dyte-breakout-rooms-manager", defaults: defaults, props: { mode: this.meeting.connectedMeetings.isActive ? 'view' : 'create' } }))));
    }
    else if (((_c = states === null || states === void 0 ? void 0 : states.activeConfirmationModal) === null || _c === void 0 ? void 0 : _c.active) === true) {
      return (index.h(index.Host, null, index.h("dyte-dialog", { open: true, onDyteDialogClose: () => this.updateStoreState('activeConfirmationModal', false), iconPack: this.iconPack, t: this.t }, index.h(index$2.Render, { element: "dyte-confirmation-modal", defaults: defaults }))));
    }
    else if (((_d = states === null || states === void 0 ? void 0 : states.activeOverlayModal) === null || _d === void 0 ? void 0 : _d.active) === true) {
      return (index.h(index.Host, null, index.h("dyte-overlay-modal", { meeting: this.meeting, states: this.states, iconPack: this.iconPack, t: this.t })));
    }
    else if (states === null || states === void 0 ? void 0 : states.activeBroadcastMessageModal) {
      return (index.h(index.Host, null, index.h("dyte-dialog", { open: true, onDyteDialogClose: () => this.updateStoreState('activeBroadcastMessageModal', false), iconPack: this.iconPack, t: this.t }, index.h(index$2.Render, { element: "dyte-broadcast-message-modal", defaults: defaults }))));
    }
      ...
      return (index.h(index.Host, null, index.h("dyte-dialog", { open: true, onDyteDialogClose: this.cancelJoinStage, iconPack: this.iconPack, t: this.t }, index.h("dyte-join-stage", Object.assign({ dataConfig: dataState, onDyteJoinStage: this.joinStage, onDyteLeaveStage: this.cancelJoinStage }, defaults)))));
    }
    else if ((states === null || states === void 0 ? void 0 : states.activeMuteAllConfirmation) === true) {
      return (index.h(index.Host, null, index.h("dyte-dialog", { open: true, onDyteDialogClose: () => {
          this.updateStoreState('activeMuteAllConfirmation', false);
        }, iconPack: this.iconPack, t: this.t }, index.h(index$2.Render, { element: "dyte-mute-all-confirmation", defaults: defaults }))));
    }
    else if (states === null || states === void 0 ? void 0 : states.activeChannelCreator) {
      return (index.h(index.Host, null, index.h("dyte-dialog", { open: true, onDyteDialogClose: () => {
          this.updateStoreState('activeChannelCreator', false);
        }, iconPack: this.iconPack, t: this.t }, index.h(index$2.Render, { element: "dyte-channel-creator", defaults: defaults }))));
    }
    return null;
  }
  static get watchers() { return {
...
const DyteNotifications = class {
  constructor(hostRef) {
    ...
    this.onRemoteUpdate = ({ payload, type }) => {
      if (type === dyteClient.RemoteUpdateType.INCOMING_REQUEST_ACCEPTED) {
      ...
      if (type === dyteClient.RemoteUpdateType.OUTGOING_REQUEST_ACCEPTED) {
      ...
      if (type === dyteClient.RemoteUpdateType.REQUEST_RECEIVED) {
        let remotePeer = this.meeting.participants.joined.get(payload.request.remotePeerId);
        this.add({
          id: `message-${Math.random().toString(36)}`,
          icon: this.iconPack.chat,
      ...
      if (type === dyteClient.RemoteUpdateType.INCOMING_REQUEST_ENDED ||
        type === dyteClient.RemoteUpdateType.OUTGOING_REQUEST_ENDED) {
        this.add({
          id: `message-${Math.random().toString(36)}`,
          icon: this.iconPack.chat,
      ...
      if (type === dyteClient.RemoteUpdateType.REQUEST_SENT) {
        let hostPeer = this.meeting.participants.joined.get(payload.request.hostPeerId);
        this.add({
          id: `message-${Math.random().toString(36)}`,
          icon: this.iconPack.chat,
  ...
  meetingChanged(meeting, oldMeeting) {
    ...
    this.participantJoinedListener = (participant) => {
      if (notifications.participant_joined) {
        this.add({
          ...
          message: `${string.formatName(participant.name)} just joined`,
    ...
    this.participantLeftListener = (participant) => {
      if (notifications.participant_left) {
        this.add({
          ...
          message: `${string.formatName(participant.name)} left`,
          image: participant.picture,
    ...
    this.waitlistedParticipantJoinedListener = (participant) => {
      ...
      this.add({
        id,
        message: `${string.formatName(participant.name)} is requesting to join the meeting`,
        image: participant.picture,
    ...
    !livestream.showLivestream(meeting) && ((_a = meeting.chat) === null || _a === void 0 ? void 0 : _a.addListener('chatUpdate', this.chatUpdateListener));
  ...
  render() {
    return (index.h(index.Host, null, this.notifications.map((notification) => (index.h("dyte-notification", { size: this.size, key: notification.id, "data-id": notification.id, notification: notification, onDyteNotificationDismiss: (e) => this.handleDismiss(e), iconPack: this.iconPack, t: this.t })))));
  }
  get host() { return index.getElement(this); }
  static get watchers() { return {
    "meeting": ["meetingChanged"],
