...
const DyteChat = class {
  constructor(hostRef) {
    ...
    this.usePaginatedChat = () => flags.usePaginatedChat(this.meeting);
    ...
    this.isPrivateChatSupported = () => {
      var _a, _b, _c;
      return (!flags.usePaginatedChat(this.meeting) &&
    ...
    this.onChannelCreateClicked = () => {
      ...
      store.state.activeChannelCreator = true;
    };
    ...
    this.renderPinnedMessagesHeader = () => {
      ...
      if (this.meeting.chat.pinned.length === 0)
        return null;
      return (index.h("dyte-tooltip", { label: this.t('chat.toggle_pinned_msgs'), iconPack: this.iconPack, t: this.t }, index.h("div", { class: { 'pinned-messages-header': true, active: this.showPinnedMessages }, onClick: this.onTogglePinnedMessages }, index.h("dyte-icon", { icon: this.iconPack.pin, iconPack: this.iconPack, t: this.t }), this.t('chat.pinned_msgs'), ` (${this.meeting.chat.pinned.length})`)));
    };
  ...
  chatGroupsChanged(chatGroups) {
    ...
    for (const key in chatGroups) {
      const lastReadTimestamp = (_a = userPrefs.chatUnreadTimestamps[key]) !== null && _a !== void 0 ? _a : 0;
      ...
      if (key ===
        ...
        unreadCounts[key] = 0;
        userPrefs.chatUnreadTimestamps[key] = new Date();
      }
    }
  ...
  renderHeadlessComponents() {
    return (index.h(index.Fragment, null, index.h("dyte-dialog-manager", { meeting: this.meeting }), index.h("dyte-notifications", { meeting: this.meeting })));
  }
  renderComposerUI() {
    var _a, _b, _c;
    ...
    return (index.h("dyte-chat-composer-view", Object.assign({ message: message, storageKey: (_a = this.selectedChannelId) !== null && _a !== void 0 ? _a : `draft-${this.selectedChannelId}`, quotedMessage: quotedMessage, isEditing: !!this.editingMessage, canSendTextMessage: this.isTextMessagingAllowed(), canSendFiles: this.isFileMessagingAllowed(), disableEmojiPicker: !!((_c = (_b = this.meeting) === null || _b === void 0 ? void 0 : _b.__internals__) === null || _c === void 0 ? void 0 : _c.features.hasFeature(flags.FlagsmithFeatureFlags.DISABLE_EMOJI_PICKER)), inputTextPlaceholder: this.t('chat.message_placeholder'), onNewMessage: this.onNewMessageHandler, onEditMessage: this.onEditMessageHandler, onEditCancel: this.onEditCancel, onQuotedMessageDismiss: this.onQuotedMessageDismiss }, uiProps), index.h("slot", { name: "chat-addon", slot: "chat-addon" })));
  }
  renderFullChat() {
    if (this.creatingChannel) {
      return (index.h("div", { class: "banner" }, index.h("dyte-spinner", { size: "lg" })));
    }
    if (this.channels.length === 0 || !this.selectedChannelId) {
      return (index.h("div", { class: "banner" }, index.h("dyte-icon", { icon: this.iconPack.create_channel_illustration, iconPack: this.iconPack, t: this.t, slot: "start", class: 'create-channel-illustration' }), index.h("dyte-button", { iconPack: this.iconPack, t: this.t, kind: "wide", variant: "primary", size: "md", onClick: this.onChannelCreateClicked, class: "welcome-new-channel" }, index.h("dyte-icon", { icon: this.iconPack.add, iconPack: this.iconPack, t: this.t, slot: "start" }), index.h("span", null, this.t('chat.new_channel'))), (this.selectorState === 'mobile' || this.selectorState === 'hide') && (index.h("dyte-button", { kind: "button", variant: "secondary", size: "md", class: "view-chats-btn", onClick: () => {
          this.selectorState = 'mobile';
        } }, index.h("dyte-icon", { icon: this.iconPack.chat, slot: "start" }), index.h("span", null, this.t('chat.view_chats'))))));
    }
    const selectedChannel = this.channels.find((channel) => channel.id === this.selectedChannelId);
    return (index.h("div", { class: "chat" }, index.h("dyte-channel-header", { slot: "header", meeting: this.meeting, channel: selectedChannel, onSearch: this.onSearchHandler, onSearchDismissed: this.onSearchDismissed, showBackButton: this.selectorState === 'mobile' || this.selectorState === 'hide', onBack: () => {
        this.selectorState = 'mobile';
      } }), this.searchQuery !== '' && (index.h("dyte-chat-search-results", { meeting: this.meeting, query: this.searchQuery, channelId: this.selectedChannelId })), this.searchQuery === '' && (index.h("dyte-chat-messages-ui-paginated", { meeting: this.meeting, size: this.size, iconPack: this.iconPack, t: this.t, selectedChannelId: this.selectedChannelId, selectedChannel: selectedChannel }))));
  }
  render() {
    var _a, _b;
    if (!this.meeting)
    ...
    return (index.h(index.Host, null, this.isChatViewType && this.renderHeadlessComponents(), index.h("div", { class: "chat-container" }, this.isChatViewType && (index.h("div", { class: { 'selector-container': true, [this.selectorState]: true } }, index.h("dyte-channel-selector-view", { channels: this.getChannelItems(), selectedChannelId: this.selectedChannelId, onChannelChange: this.onChannelChanged, t: this.t }, index.h("div", { class: "channel-selector-header", slot: "header" }, index.h("dyte-logo", { meeting: this.meeting, config: this.config, t: this.t }), index.h("dyte-tooltip", { label: this.t('chat.new_channel'), iconPack: this.iconPack, t: this.t }, index.h("dyte-button", { iconPack: this.iconPack, t: this.t, kind: "button", variant: "ghost", size: "md", onClick: this.onChannelCreateClicked, class: "channel-create-btn" }, index.h("dyte-icon", { icon: this.iconPack.add, iconPack: this.iconPack, t: this.t }))))), index.h("dyte-button", { kind: "icon", variant: "ghost", class: "mobile-close-btn", onClick: () => (this.selectorState = 'hide') }, index.h("dyte-icon", { icon: this.iconPack.dismiss })))), index.h("div", { class: "chat" }, this.isFileMessagingAllowed() && (index.h("div", { id: "dropzone", class: { active: this.dropzoneActivated }, part: "dropzone" }, index.h("dyte-icon", { icon: this.iconPack.attach, iconPack: this.iconPack, t: this.t }), index.h("p", null, this.t('chat.send_attachment')))), this.renderPinnedMessagesHeader(), this.isPrivateChatSupported() && (index.h("dyte-channel-selector-view", { channels: this.getPrivateChatRecipients(), selectedChannelId: ((_b = this.selectedParticipant) === null || _b === void 0 ? void 0 : _b.userId) || 'everyone', onChannelChange: this.updateRecipients, t: this.t, viewAs: "dropdown" })), this.isChatViewType ? (this.renderFullChat()) : this.usePaginatedChat() ? (index.h("dyte-chat-messages-ui-paginated", { meeting: this.meeting, size: this.size, iconPack: this.iconPack, t: this.t })) : (index.h("dyte-chat-messages-ui", Object.assign({ messages: chatMessages, selfUserId: selfUserId, selectedGroup: this.selectedGroup, onPinMessage: this.onPinMessage, canPinMessages: this.canPinMessages }, uiProps))), this.renderComposerUI()))));
  }
