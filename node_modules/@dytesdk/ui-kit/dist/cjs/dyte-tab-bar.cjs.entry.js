...
const DyteTabBar = class {
  ...
  render() {
    return (index.h(index.Host, null, index.h("dyte-spotlight-indicator", { meeting: this.meeting, iconPack: this.iconPack, t: this.t, size: this.size }), this.tabs.map((tab) => {
      var _a, _b, _c, _d, _e;
      let isActive = false;
      ...
      if (tab.type === 'screenshare') {
        ...
        return (index.h("dyte-button", { title: `${name}'s Screen Share`, key: tab.participant.id, kind: "icon", iconPack: this.iconPack, t: this.t, variant: isActive ? 'primary' : 'secondary', class: {
            tab: true,
            active: isActive,
          }, onClick: () => this.tabChange.emit(tab) }, index.h("div", { class: "center col" }, index.h("dyte-icon", { icon: this.iconPack.share_screen_person, iconPack: this.iconPack, t: this.t }), index.h("span", { class: "name" }, participant.id === ((_e = this.meeting) === null || _e === void 0 ? void 0 : _e.self.id) ? this.t('you') : string.shorten(name, 6)))));
      }
      else if (tab.type === 'plugin') {
        const plugin = tab.plugin;
        return (index.h("dyte-button", { title: plugin.name, key: plugin.id, kind: "icon", iconPack: this.iconPack, t: this.t, variant: isActive ? 'primary' : 'secondary', class: {
            tab: true,
            active: isActive,
          }, onClick: () => this.tabChange.emit(tab) }, index.h("div", { class: "center col" }, index.h("img", { src: plugin.picture }), index.h("span", { class: "name" }, string.shorten(plugin.name, 6)))));
      }
    })));
  }
};
