...
const debounce = require('./debounce-ecc75553.js');
...
const DyteGrid = class {
  constructor(hostRef) {
    ...
    this.stageStatusListener = () => {
      this.updateActiveParticipants();
      this.showLiveStreamPlayer = livestream.isLiveStreamViewer(this.meeting);
  ...
  screenShareParticipantsChanged(participants) {
    ...
    if (!!store.state.activeScreenShare === activeScreenShare)
      return;
    this.stateUpdate.emit({ activeScreenShare });
    store.state.activeScreenShare = activeScreenShare;
  }
  pluginsChanged(plugins) {
    const activePlugin = plugins.length > 0;
    if (!!store.state.activePlugin === activePlugin)
      return;
    this.stateUpdate.emit({ activePlugin });
    store.state.activePlugin = activePlugin;
  }
  pinnedParticipantsChanged(participants) {
    const activeSpotlight = participants.length > 0;
    if (!!store.state.activeSpotlight === activeSpotlight)
      return;
    this.stateUpdate.emit({ activeSpotlight });
    store.state.activeSpotlight = activeSpotlight;
  }
  ...
  render() {
    ...
    if (this.roomState !== 'joined' && !livestream.isLiveStreamViewer(this.meeting))
      return (index.h(index.Host, null, index.h("div", { class: "offline-grid" }, index.h("dyte-icon", { icon: this.iconPack.join_stage, size: "xl" }), index.h("h3", null, this.t('disconnected')), index.h("p", null, this.t('disconnected.description')))));
    if (!this.mediaConnected && !livestream.isLiveStreamViewer(this.meeting))
      return (index.h(index.Host, null, index.h("div", { class: "offline-grid" }, index.h("dyte-icon", { icon: this.iconPack.disconnected, size: "xl" }), index.h("h3", null, this.t('offline')), index.h("p", null, this.t('offline.description')))));
    if (!this.meetingConnected && this.hideSelf && !livestream.isLiveStreamViewer(this.meeting)) {
      return (index.h("div", { class: "offline-grid" }, index.h("p", null, this.t('stage.reconnecting'))));
    }
    if (this.showStage && !this.showLiveStreamPlayer) {
      return (index.h(index.Host, null, index.h("div", { class: "webinar-stage" }, index.h("div", { class: "center" }, this.canCurrentPeerHost && (index.h("div", { class: "ctr", part: "container" }, index.h("p", { class: "message", part: "message" }, this.t('stage.empty_host')), index.h("span", { class: "description", part: "description" }, this.t('stage.empty_host_summary')))), !this.canCurrentPeerHost && (index.h("div", { class: "ctr", part: "container" }, index.h("p", { class: "message", part: "message" }, this.t('stage.empty_viewer'))))))));
    }
    if (this.showLiveStreamPlayer) {
      return (index.h(index.Host, null, index.h("dyte-livestream-player", { meeting: this.meeting, size: this.size }), index.h("dyte-livestream-indicator", { meeting: this.meeting, size: "sm", t: this.t }), index.h("dyte-viewer-count", { meeting: this.meeting, variant: "embedded", t: this.t })));
    }
    return (index.h(index.Host, null, index.h(index$2.Render, { element: "dyte-grid", defaults: defaults, childProps: {
      ...
      }, onlyChildren: true }), index.h("dyte-livestream-indicator", { meeting: this.meeting, size: "sm", t: this.t }), index.h("dyte-viewer-count", { meeting: this.meeting, variant: "embedded" })));
  }
