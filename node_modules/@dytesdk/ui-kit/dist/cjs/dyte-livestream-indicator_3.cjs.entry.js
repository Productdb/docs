...
const DyteLivestreamIndicator = class {
  ...
  render() {
    if (!livestream.showLivestream(this.meeting) || !this.isLivestreaming)
      return;
    return (index.h(index.Host, null, index.h("div", { class: "indicator", "aria-label": this.t('livestream.indicator'), part: "indicator" }, index.h("dyte-icon", { icon: this.iconPack.start_livestream, size: this.size }), index.h("span", null, this.t('LIVE')))));
  }
...
const DyteLivestreamPlayer = class {
  constructor(hostRef) {
    ...
    this.getLoadingState = () => {
      ...
      switch (this.livestreamState) {
        ...
        case 'STOPPING':
          ...
          break;
        case 'LIVESTREAMING':
          if (this.playerState !== livestream.PlayerState.PLAYING) {
            loadingMessage = this.t('livestream.starting');
            showIcon = true;
            isLoading = true;
          }
    ...
    this.playerState = livestream.PlayerState.IDLE;
  ...
  AddPlayerListeners(player = this.ivsPlayer) {
    Object.values(Object.assign(Object.assign({}, livestream.PlayerEventType), livestream.PlayerState)).forEach((key) => {
      if (livestream.awsIvsPlayerEventsToIgnore.includes(key))
        return;
      player === null || player === void 0 ? void 0 : player.addEventListener(key, (event) => {
        var _a, _b, _c;
        if (key === livestream.PlayerState.IDLE ||
          key === livestream.PlayerState.PLAYING ||
          key === livestream.PlayerEventType.ERROR ||
          key === livestream.PlayerState.READY)
          this.playerState = key;
        if (key === livestream.PlayerEventType.ERROR) {
          this.playerError = event;
        }
        if (key === livestream.PlayerState.IDLE && player.isPaused()) {
          player.play();
        }
        if (key === livestream.PlayerEventType.AUDIO_BLOCKED) {
        ...
        if (livestream.isIvsPlayerCallStatsEvent.includes(key)) {
  ...
  render() {
    if (!livestream.showLivestream(this.meeting))
      return;
    const { isError, errorMessage } = this.getErrorState();
    const { isLoading, loadingMessage, showIcon } = this.getLoadingState();
    return (index.h(index.Host, null, index.h("div", { class: "player-container" }, index.h("video", { ref: (el) => this.onPlayerRef(el), playsInline: true }), this.audioPlaybackError && (index.h("div", { class: "unmute-popup" }, index.h("h3", null, this.t('audio_playback.title')), index.h("p", null, this.t('audio_playback.description')), index.h("dyte-button", { kind: "wide", onClick: () => {
        this.player.muted = false;
        this.audioPlaybackError = false;
      }, title: this.t('audio_playback'), iconPack: this.iconPack, t: this.t }, this.t('audio_playback')))), this.livestreamState === 'LIVESTREAMING' && this.showLatencyIndicator && (index.h("div", { class: "latency-controls" })), isError && (index.h("div", { class: "loader" }, index.h("dyte-icon", { icon: this.iconPack.warning, t: this.t }), index.h("p", null, errorMessage))), !isError && isLoading && (index.h("div", { class: "loader" }, showIcon && (index.h("dyte-spinner", { id: "icon", part: "spinner", iconPack: this.iconPack, t: this.t, size: "md" })), index.h("p", null, loadingMessage))))));
  }
  static get watchers() { return {
    "meeting": ["meetingChanged"]
  }; }
...
const DyteViewerCount = class {
  ...
  render() {
    if (!livestream.showLivestream(this.meeting))
      return null;
    return (index.h(index.Host, { tabIndex: 0, role: "log", "aria-label": `${this.viewerCount} ${this.t('viewers')}` }, index.h("dyte-icon", { icon: this.iconPack.viewers, tabIndex: -1, "aria-hidden": true, part: "icon", iconPack: this.iconPack, t: this.t }), this.viewerCount, " ", this.t('viewers')));
  }
  static get watchers() { return {
    "meeting": ["meetingChanged"]
  }; }
...
exports.dyte_livestream_indicator = DyteLivestreamIndicator;
exports.dyte_livestream_player = DyteLivestreamPlayer;
exports.dyte_viewer_count = DyteViewerCount;
