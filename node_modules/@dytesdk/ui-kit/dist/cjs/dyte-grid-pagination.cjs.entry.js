...
const DyteGridPagination = class {
  constructor(hostRef) {
    ...
    this.toggleGridPagination = debounce.debounce_1(() => {
      var _a;
    ...
    this.onParticipantJoin = debounce.debounce_1(() => {
      this.pageCount = this.meeting.participants.pageCount;
      this.toggleGridPagination();
    }, MASS_ACTIONS_DEBOUNCE_TIMER);
    this.onParticipantLeave = debounce.debounce_1(() => {
      this.pageCount = this.meeting.participants.pageCount;
      this.toggleGridPagination();
    }, MASS_ACTIONS_DEBOUNCE_TIMER);
    this.onStateStatusUpdate = debounce.debounce_1(() => {
      this.pageCount = this.meeting.participants.pageCount;
      this.toggleGridPagination();
    }, MASS_ACTIONS_DEBOUNCE_TIMER);
    this.prevPage = () => {
  ...
  render() {
    ...
    return (index.h(index.Host, null, index.h("dyte-button", { class: "prev", variant: "secondary", kind: "icon", disabled: this.pageCount === 0, onClick: this.prevPage, "aria-label": this.t('page.prev'), iconPack: this.iconPack, t: this.t }, index.h("dyte-icon", { icon: this.iconPack.chevron_left, iconPack: this.iconPack, t: this.t })), this.variant !== 'grid' && (index.h("div", { class: "center" }, index.h("span", { class: "page" }, this.pageCount === 0 ? (index.h("dyte-tooltip", { label: this.t('layout.auto'), iconPack: this.iconPack, t: this.t }, index.h("dyte-button", { kind: "icon", class: "auto", iconPack: this.iconPack, t: this.t }, index.h("dyte-icon", { icon: this.iconPack.wand, iconPack: this.iconPack, t: this.t })))) : (this.page)), this.pageCount !== 0 && [
      index.h("span", { class: "slash" }, "/"),
      index.h("span", { class: "pages" }, this.pageCount),
    ])), this.variant === 'grid' && this.pageCount > 0 && (index.h("div", { class: "dots" }, [...Array(this.pageCount)].map((_, index$1) => (index.h("div", { key: `dot-${index$1}`, class: { dot: true, active: index$1 + 1 === this.page } }))))), index.h("dyte-button", { class: "next", variant: "secondary", kind: "icon", disabled: this.page !== 0 && this.page === this.pageCount, onClick: this.nextPage, "aria-label": this.t('page.next'), iconPack: this.iconPack, t: this.t }, index.h("dyte-icon", { icon: this.iconPack.chevron_right, tabIndex: -1, "aria-hidden": true, iconPack: this.iconPack, t: this.t }))));
  }
...
exports.dyte_grid_pagination = DyteGridPagination;
