...
const DyteChatComposerUi = class {
  constructor(hostRef) {
    ...
    this.onNewMessage = index.createEvent(this, "dyteNewMessage", 7);
    this.onEditMessage = index.createEvent(this, "dyteEditMessage", 7);
    this.onEditCancelled = index.createEvent(this, "dyteEditCancelled", 7);
    ...
    this.initializeTextField = (el) => {
      this.$textArea = el;
      const message = userPrefs.gracefulStorage.getItem(this.storageKey) || '';
    ...
    this.onMemberSelect = (member) => {
      ...
      this.mentionQuery = '';
      this.focusedMemberIndex = 0;
      index.writeTask(() => this.$textArea.focus());
    };
    ...
    this.renderSuggestedReplies = () => {
      ...
      if (this.prefill.suggestedReplies.length === 0)
        return;
      return (index.h("ul", { class: "suggested-replies scrollbar" }, this.prefill.suggestedReplies.map((reply) => (index.h("dyte-tooltip", { label: this.t('chat.click_to_send') }, index.h("li", { onClick: () => this.onNewMessage.emit({ type: 'text', message: reply }) }, reply))))));
    };
    this.renderMenu = () => {
      ...
      if (filteredMembers.length === 0)
        return;
      return (index.h("ul", { class: "member-list scrollbar" }, filteredMembers.map((member, index$1) => (index.h("li", { class: { member: true, selected: index$1 === this.focusedMemberIndex }, onClick: () => this.onMemberSelect(member), ref: ($li) => {
          if (index$1 === this.focusedMemberIndex) {
            index.writeTask(() => {
              if ($li)
                $li.scrollIntoView({ behavior: 'smooth', block: 'end', inline: 'nearest' });
            });
          }
        } }, index.h("dyte-avatar", { participant: {
          name: member.name,
          picture: member.picture,
        }, size: "sm" }), index.h("span", null, member.name))))));
    };
  ...
  renderFilePreview() {
    if (typeof this.filePreview !== 'string')
      return;
    return (index.h("div", { class: "preview-overlay" }, index.h("div", { class: "file-preview" }, index.h("dyte-tooltip", { label: this.t('chat.cancel_upload') }, index.h("dyte-button", { variant: "secondary", kind: "icon", onClick: this.cleanUpFileUpload }, index.h("dyte-icon", { icon: this.iconPack.dismiss }))), this.fileToUpload.type === 'image' ? (index.h("img", { class: "preview-image", src: this.filePreview })) : (index.h("div", { class: "preview-file" }, index.h("span", null, this.filePreview))))));
  }
  render() {
    var _a;
    ...
    return (index.h(index.Host, null, this.canSendTextMessage && this.emojiPickerActive && (index.h("dyte-emoji-picker", { part: "emoji-picker", onPickerClose: () => {
        this.emojiPickerActive = false;
      }, onDyteEmojiClicked: (e) => {
        this.$textArea.value += e.detail;
        this.$textArea.focus();
      }, t: this.t })), this.renderSuggestedReplies(), index.h("slot", { name: "chat-addon" }), index.h("slot", { name: "quote-block" }), index.h("div", { class: "chat-input", part: "chat-input" }, this.renderMenu(), this.canSendTextMessage && (index.h("textarea", { class: "scrollbar", part: "textarea", ref: this.initializeTextField, autoFocus: true, placeholder: this.fileToUpload ? '' : this.t('chat.message_placeholder'), value: defaultValue, onPaste: this.onPaste, maxLength: chat.MAX_TEXT_LENGTH, onKeyDown: this.handleKeyDown, onKeyUp: this.handleKeyUp, onInput: (e) => {
        userPrefs.gracefulStorage.setItem(this.storageKey, e.target.value);
      }, disabled: !!this.filePreview })), index.h("div", { class: "chat-buttons", part: "chat-buttons" }, index.h("div", { class: "left", part: "chat-buttons-left" }, !this.prefill.editMessage &&
      this.canSendFiles && [
      index.h("dyte-tooltip", Object.assign({ label: this.t('chat.send_file') }, uiProps), index.h("dyte-button", { variant: "ghost", kind: "icon", onClick: () => this.uploadFile('file'), title: this.t('chat.send_file'), iconPack: this.iconPack, t: this.t }, index.h("dyte-icon", { icon: this.iconPack.attach }))),
      index.h("dyte-tooltip", Object.assign({ label: this.t('chat.send_img') }, uiProps), index.h("dyte-button", { variant: "ghost", kind: "icon", onClick: () => this.uploadFile('image'), title: this.t('chat.send_img'), iconPack: this.iconPack, t: this.t }, index.h("dyte-icon", { icon: this.iconPack.image }))),
    ], !this.prefill.editMessage && this.canSendTextMessage && !this.disableEmojiPicker && (index.h("dyte-tooltip", Object.assign({ label: this.t('chat.send_emoji') }, uiProps), index.h("dyte-button", { variant: "ghost", kind: "icon", class: { active: this.emojiPickerActive }, title: this.t('chat.send_emoji'), iconPack: this.iconPack, t: this.t, onClick: () => {
        this.emojiPickerActive = !this.emojiPickerActive;
      } }, index.h("dyte-icon", { icon: this.iconPack.emoji_multiple }))))), !!this.filePreview && this.renderFilePreview(), this.canSendTextMessage && (index.h("div", { class: "right", part: "chat-buttons-right" }, !this.prefill.editMessage && (index.h("dyte-tooltip", Object.assign({ variant: "primary", label: this.t('chat.send_msg'), delay: 2000 }, uiProps), index.h("dyte-button", { kind: "icon", onClick: () => this.handleSendMessage(), title: this.t('chat.send_msg'), iconPack: this.iconPack, t: this.t }, index.h("dyte-icon", { icon: this.iconPack.send })))), this.prefill.editMessage && (index.h("div", { class: "edit-buttons" }, index.h("dyte-tooltip", Object.assign({ variant: "secondary", label: this.t('cancel'), delay: 2000 }, uiProps), index.h("dyte-button", { kind: "icon", variant: "secondary", onClick: () => this.handleEditCancel(), title: this.t('cancel'), iconPack: this.iconPack, t: this.t }, index.h("dyte-icon", { icon: this.iconPack.dismiss }))), index.h("dyte-tooltip", Object.assign({ variant: "primary", label: this.t('chat.update_msg'), delay: 2000 }, uiProps), index.h("dyte-button", { kind: "icon", onClick: () => this.handleEditMessage(), title: this.t('chat.send_msg'), iconPack: this.iconPack, t: this.t }, index.h("dyte-icon", { icon: this.iconPack.checkmark })))))))))));
  }
...
exports.dyte_chat_composer_ui = DyteChatComposerUi;
