...
const DyteFileMessageView = class {
  ...
  render() {
    return (index.h("div", { class: "file" }, index.h("dyte-button", { variant: "secondary", kind: "icon", iconPack: this.iconPack, t: this.t, onClick: () => file.downloadFile(string.sanitizeLink(this.url), { name: this.name, fallbackName: 'file' }), part: "button" }, index.h("dyte-icon", { icon: this.iconPack.download, iconPack: this.iconPack, t: this.t })), index.h("div", { class: "file-data" }, index.h("div", { class: "name" }, this.name), index.h("div", { class: "file-data-split" }, index.h("div", { class: "ext" }, file.getExtension(this.name)), index.h("span", { class: "divider" }), index.h("div", { class: "size" }, file.getFileSize(this.size))))));
  }
};
...
const DyteImageMessageView = class {
  constructor(hostRef) {
    index.registerInstance(this, hostRef);
    this.onPreview = index.createEvent(this, "preview", 7);
    this.url = undefined;
    this.iconPack = defaultIconPack.defaultIconPack;
    this.t = index$1.useLanguage();
    this.status = 'loading';
  ...
  render() {
    return (index.h("div", { class: { image: true, loaded: this.status === 'loaded' } }, index.h("img", { src: string.sanitizeLink(this.url), onLoad: () => {
        this.status = 'loaded';
      }, onError: () => {
        this.status = 'errored';
      }, onClick: () => {
...
const DyteMessageView = class {
  constructor(hostRef) {
    index.registerInstance(this, hostRef);
    this.onAction = index.createEvent(this, "action", 7);
    this.actions = [];
    this.variant = 'bubble';
  ...
  renderActions() {
    return (index.h("dyte-menu", { placement: "top-end", offset: 1 }, index.h("button", { slot: "trigger", class: "actions" }, index.h("dyte-icon", { icon: this.iconPack.chevron_down })), index.h("dyte-menu-list", null, this.actions.map((action) => (index.h("dyte-menu-item", { onClick: () => this.onAction.emit(action.id) }, action.icon && index.h("dyte-icon", { icon: action.icon, slot: "start" }), action.label))))));
  }
  render() {
    return (index.h(index.Host, null, index.h("div", { class: { 'message-wrapper': true, [this.viewType]: true } }, !this.hideAvatar && (index.h("aside", { class: "avatar", part: "avatar" }, index.h("dyte-avatar", { participant: { name: this.authorName, picture: this.avatarUrl }, size: "sm" }))), index.h("div", { class: "message", part: "message" }, !this.hideAuthorName && index.h("div", { class: "header" }, this.authorName), index.h("div", { class: { body: true, bubble: this.variant === 'bubble' } }, index.h("slot", null), !this.hideMetadata && !!this.time && (index.h("div", { class: "metadata", title: date.formatDateTime(this.time) }, date.elapsedDuration(this.time, new Date(Date.now())))), this.actions.length !== 0 && this.renderActions())))));
  }
};
