...
const DyteBreakoutRoomsManager = class {
  constructor(hostRef) {
    ...
    this.enableConfirmationModal = (modalType = 'start-breakout') => {
      ...
      store.state.activeBreakoutRoomsManager = { active: false };
      store.state.activeConfirmationModal = activeConfirmationModal;
    };
    this.close = () => {
      var _a;
  ...
  connectedCallback() {
    ...
    this.stateManager = new breakoutRoomsManager.BreakoutRoomsManager();
    this.fetchRoomState();
  }
  ...
  onParticipantDelete(event) {
    const { detail } = event;
    const id = breakoutRooms.participantIdentifier(detail);
    if (id == null)
      return;
    this.unassignParticipant(id);
  }
  ...
  async joinRoom(destinationMeetingId) {
    const participantId = breakoutRooms.participantIdentifier(this.meeting.self);
  ...
  renderMainRoomMaybe() {
    ...
    return (index.h("dyte-breakout-room-manager", { key: this.stateManager.currentState.parentMeeting['id'], assigningParticipants: this.assigningParticipants, isDragMode: this.isDragMode, meeting: this.meeting, mode: this.roomConfig.mode, onParticipantsAdd: () => this.assignParticipantsToRoom(this.stateManager.currentState.parentMeeting['id']), onRoomJoin: () => this.joinRoom(this.stateManager.currentState.parentMeeting['id']), onUpdate: this.onRoomUpdate, states: this.states, room: Object.assign({}, this.stateManager.currentState.parentMeeting), iconPack: this.iconPack, t: this.t }));
  }
  renderRoomSwitcher() {
    return (index.h(index.Host, null, index.h("div", { class: "room-switcher-container" }, index.h("header", null, index.h("dyte-icon", { icon: this.iconPack.breakout_rooms }), index.h("span", null, this.t('breakout_rooms.join_breakout_header'))), index.h("div", { class: "rooms", ref: (el) => (this.selectorRef = el) }, this.renderMainRoomMaybe(), this.getPermittedRooms().map((room, idx) => {
      return (index.h("dyte-breakout-room-manager", { key: room['id'], assigningParticipants: this.assigningParticipants, isDragMode: this.isDragMode, defaultExpanded: idx === 0, meeting: this.meeting, mode: this.roomConfig.mode, onDelete: () => this.onRoomDelete(room['id']), onParticipantsAdd: () => this.assignParticipantsToRoom(room['id']), onRoomJoin: () => this.joinRoom(room['id']), states: this.states, room: Object.assign({}, room), iconPack: this.iconPack, t: this.t, allowDelete: false }));
    })))));
  }
  renderLoading() {
    return (index.h(index.Host, null, index.h("div", { class: "loading-content" }, index.h("dyte-spinner", { size: "xl" }))));
  }
  renderRoomConfig() {
    return (index.h(index.Host, null, index.h("div", { class: "room-config" }, index.h("header", null, index.h("dyte-icon", { icon: this.iconPack.breakout_rooms }), index.h("span", null, this.t('breakout_rooms.room_config_header'))), index.h("div", { class: "create-room" }, index.h("p", null, this.t('breakout_rooms.num_of_rooms')), index.h("dyte-counter", { value: this.roomConfig.rooms, minValue: MIN_ROOMS, iconPack: this.iconPack, t: this.t, onValueChange: (val) => {
        this.roomConfig = Object.assign(Object.assign({}, this.roomConfig), { rooms: Math.max(+val.detail, MIN_ROOMS) });
      } })), index.h("span", { class: "distribution-hint" }, `${this.t('breakout_rooms.approx')}${' '}`, ' ', index.h("em", null, this.getApproxDistribution(), " ", this.t('breakout_rooms.participants_per_room')), ' ', this.t('breakout_rooms.division_text')), index.h("footer", null, index.h("dyte-button", { kind: "button", iconPack: this.iconPack, t: this.t, size: "lg", title: this.t('create'), disabled: this.roomConfig.rooms === 0, onClick: () => this.onCreateRooms() }, this.t('create'))))));
  }
  render() {
    if (this.loading) {
    ...
    // participant config
    return (index.h(index.Host, null, index.h("div", { class: "participant-config-wrapper" }, index.h("div", { class: "participant-config" }, index.h("aside", { part: "menu" }, index.h("header", null, this.t('breakout_rooms.assign_participants')), index.h("dyte-breakout-room-participants", { meeting: this.meeting, iconPack: this.iconPack, t: this.t, participantIds: this.stateManager.unassignedParticipants.map(breakoutRooms.participantIdentifier), selectedParticipantIds: this.selectedParticipants }, index.h("dyte-tooltip", { label: this.t('breakout_rooms.shuffle_participants'), iconPack: this.iconPack, t: this.t, slot: "shuffle-button" }, index.h("dyte-button", { disabled: this.roomConfig.mode === 'edit' ||
        this.stateManager.unassignedParticipants.length === 0, iconPack: this.iconPack, t: this.t, kind: "button", variant: "secondary", size: "md", onClick: () => this.assignParticipantsRandomly(), class: "shuffle-button br-primary-btn" }, index.h("dyte-icon", { icon: this.iconPack.shuffle, iconPack: this.iconPack, t: this.t })))), this.selectedParticipants.length !== 0 && (index.h("div", { class: "participants-assign-actions" }, index.h("span", null, `${this.selectedParticipants.length} ${this.t('breakout_rooms.selected')}`), index.h("dyte-button", { disabled: this.roomConfig.mode === 'edit', iconPack: this.iconPack, t: this.t, kind: "button", variant: "ghost", size: "md", onClick: () => this.deselectAll(), class: "deselect-button color-danger" }, this.t('breakout_rooms.deselect'))))), index.h("div", { class: "assign-rooms" }, index.h("div", { class: "row" }, index.h("p", { class: "row-header" }, this.t('breakout_rooms.rooms'), " (", this.stateManager.allConnectedMeetings.length, ")"), !this.assigningParticipants && (index.h("div", { class: "cta-buttons" }, index.h("dyte-button", { kind: "button", variant: "secondary", iconPack: this.iconPack, t: this.t, class: "br-primary-btn" }, index.h("div", { onClick: this.onAddNewRoom }, index.h("dyte-icon", { icon: this.iconPack.add, iconPack: this.iconPack, t: this.t }), this.t('breakout_rooms.add_room'))), this.stateManager.allConnectedMeetings.flatMap((m) => m.participants)
      .length !== 0 && (index.h("dyte-button", { kind: "button", variant: "ghost", onClick: this.onUnassignAll, iconPack: this.iconPack, t: this.t }, this.t('breakout_rooms.unassign_all')))))), index.h("div", { class: "rooms", ref: (el) => (this.selectorRef = el) }, this.renderMainRoomMaybe(), this.getPermittedRooms().map((room, idx) => {
    ...
    }), index.h("dyte-button", { kind: "button", variant: "secondary", iconPack: this.iconPack, t: this.t, onClick: this.onAddNewRoom, class: "add-room-jumbo-btn br-secondary-btn" }, index.h("div", null, index.h("dyte-icon", { icon: this.iconPack.add, iconPack: this.iconPack, t: this.t }), index.h("span", null, this.t('breakout_rooms.add_room_brief'))))))), index.h("div", { class: "participant-config-actions" }, index.h("div", { class: { 'status-bar': true, 'ephemeral-status': this.ephemeralStatusText !== '' } }, this.ephemeralStatusText !== '' && (index.h("dyte-icon", { icon: this.iconPack.checkmark, iconPack: this.iconPack, t: this.t })), this.getStatusText()), index.h("div", { class: "breakout-actions" }, this.roomConfig.mode === 'create' && this.permissions.canAlterConnectedMeetings && (index.h("dyte-button", { size: "md", iconPack: this.iconPack, t: this.t, class: "start-breakout-button", onClick: () => this.enableConfirmationModal('start-breakout') }, this.t('breakout_rooms.start_breakout'))), this.roomConfig.mode === 'edit' &&
      this.stateManager.hasLocalChanges &&
      this.permissions.canAlterConnectedMeetings && (index.h("dyte-button", { size: "md", iconPack: this.iconPack, t: this.t, class: "color-danger", variant: "ghost", onClick: () => this.discardChanges() }, this.t('breakout_rooms.discard_changes'))), this.roomConfig.mode === 'edit' &&
      this.stateManager.hasLocalChanges &&
      this.permissions.canAlterConnectedMeetings && (index.h("dyte-button", { size: "md", iconPack: this.iconPack, t: this.t, class: "update-breakout-button", onClick: this.applyChanges }, this.t('breakout_rooms.update_breakout'))), this.roomConfig.mode === 'edit' &&
      !this.stateManager.hasLocalChanges &&
      this.permissions.canAlterConnectedMeetings && (index.h("dyte-button", { size: "md", iconPack: this.iconPack, t: this.t, class: "close-breakout-button", onClick: () => this.enableConfirmationModal('close-breakout') }, this.t('breakout_rooms.close_breakout'))))))));
  }
