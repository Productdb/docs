...
const DyteChatSelectorUi = class {
  constructor(hostRef) {
    ...
    this.groupChanged = index.createEvent(this, "dyteChatGroupChanged", 7);
  ...
  render() {
    ...
    return (index.h(index.Host, null, index.h("div", { class: "chat-scope-selector" }, this.showParticipantsPanel && (index.h("div", { class: "participants-container" }, index.h("div", { class: "search", part: "search" }, index.h("dyte-icon", { icon: this.iconPack.search, part: "search-icon" }), index.h("input", { type: "search", autocomplete: "off", placeholder: "Search", value: this.query, onInput: (e) => {
        this.query = e.target.value;
      }, part: "search-input" })), index.h("ul", { class: "scope-list scrollbar" }, this.query === '' && (index.h("li", { class: "scope scope-special", onClick: () => this.onScopeClick('everyone') }, index.h("div", { class: "everyone-icon" }, index.h("dyte-icon", { icon: this.iconPack.participants })), "Everyone", this.unreadCounts['everyone'] > 0 && (index.h("div", { class: "unread-count", part: "unread-count" }, index.h("span", null, this.unreadCounts['everyone']))))), this.getGroups().map((group) => {
      const count = this.unreadCounts[chat.generateChatGroupKey([this.selfUserId, group.userId])];
      return (index.h("li", { class: "scope", onClick: () => this.onScopeClick(group), key: group.userId }, group.name, count > 0 && (index.h("div", { class: "unread-count", part: "unread-count" }, index.h("span", null, count)))));
    })))), index.h("button", { onClick: () => this.toggleParticipants() }, index.h("span", null, `${this.t('to')} ${this.getName()}`, '  ', unreadCountTotal > 0 && (index.h("div", { class: "unread-count selector" }, index.h("span", null, unreadCountTotal)))), index.h("dyte-icon", { icon: this.showParticipantsPanel ? this.iconPack.chevron_up : this.iconPack.chevron_down })))));
  }
};
DyteChatSelectorUi.style = dyteChatSelectorUiCss;

exports.dyte_chat_selector_ui = DyteChatSelectorUi;
