...
const DyteParticipantTile = class {
  constructor(hostRef) {
    ...
    this.tileLoad = index.createEvent(this, "tileLoad", 7);
    this.tileUnload = index.createEvent(this, "tileUnload", 7);
    ...
    this.onPause = (event) => {
      var _a, _b;
      if (this.isSelf() &&
        ((_a = this.meeting) === null || _a === void 0 ? void 0 : _a.__internals__.features.hasFeature(flags.FlagsmithFeatureFlags.PLAY_PARTICIPANT_TILE_VIDEO_ON_PAUSE))) {
  ...
  videoStateChanged(videoState) {
    ...
    if (videoState != null && this.videoEl != null) {
      if (videoState.videoEnabled) {
        if ((_a = this.meeting) === null || _a === void 0 ? void 0 : _a.__internals__.features.hasFeature(flags.FlagsmithFeatureFlags.LOG_PLAYING_FAILURES)) {
          if (this.playTimeout)
            clearTimeout(this.playTimeout);
  ...
  render() {
    ...
    return (index.h(index.Host, { class: { hideAvatar: this.videoState.videoEnabled } }, index.h("video", { ref: (el) => this.onVideoRef(el), class: {
      ...
      }, onPlaying: this.onPlaying, onPause: this.onPause, autoPlay: true, playsInline: true, muted: true, part: "video" }), this.isPinned && (index.h("dyte-icon", { class: "pinned-icon", icon: this.iconPack.pin, "aria-label": this.t('pinned'), iconPack: this.iconPack, t: this.t, part: "pinned-icon" })), index.h("slot", null, index.h(index$2.Render, { element: "dyte-participant-tile", defaults: defaults, childProps: {
