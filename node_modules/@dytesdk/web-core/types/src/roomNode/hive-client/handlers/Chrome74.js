...
const logger = Logger.getLogger('Chrome74');
export class Chrome74 extends HandlerInterface {
    ...
    init({ direction, iceServers, iceTransportPolicy, additionalSettings, proprietaryConstraints, onTrackHandler, }) {
        logger.debug('init()', additionalSettings);
    ...
    async send({ track, encodings, codecOption, screenShare, }) {
        ...
        const sendingCapabilities = RTCRtpSender.getCapabilities(track.kind);
        logger.info('senders available params:', sendingCapabilities);
        const selectedCodecs = [];
        if (codecOption) {
            const codec = sendingCapabilities.codecs.find((c) => c.mimeType.includes(codecOption.name));
            if (codecOption.parameters) {
                logger.info('codecOption.parameters:', codecOption.parameters);
                const fmtps = codec.sdpFmtpLine?.split(';') || [];
                fmtps.push(...codecOption.parameters);
                const uniqueFmtps = [...new Set(fmtps).values()];
                codec.sdpFmtpLine = uniqueFmtps.join(';');
            }
            selectedCodecs.push(codec);
        }
        logger.info('selected codecs:', selectedCodecs);
        trans.setCodecPreferences(selectedCodecs);
        const offer = await this._pc.createOffer();
        if (!this._transportReady)
            throw new Error('webrtc transport not connected');
        await this._pc.setLocalDescription(offer);
        if (codecOption && codecOption.name === 'opus') {
            offer.sdp = offer.sdp.replace('minptime=10;useinbandfec=1', 'minptime=10;useinbandfec=1;usedtx=1');
            offer.sdp += 'a=rtcp-fb:111 nack\r\n';
        }
    ...
    _addEventListeners() {
        ...
        this._pc.addEventListener('icecandidateerror', (e) => {
            logger.info('icecandidateerror', e);
        });
        this._pc.addEventListener('datachannel', (event) => {
            ...
            channel.onmessage = (msg) => {
                logger.info('data channel message: ', event.channel.label, msg);
                this.safeEmit('datachannel', event.channel, String.fromCharCode.apply(null, new Uint8Array(msg.data)));
            };
        });
    }
}
