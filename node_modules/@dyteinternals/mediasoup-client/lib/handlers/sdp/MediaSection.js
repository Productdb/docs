...
export class MediaSection {
    ...
    muxSimulcastStreams(encodings) {
        if (this._mediaObject.simulcast && this._mediaObject.simulcast.list1) {
            ...
            // @ts-ignore
            const parsedList = sdpTransform.parseSimulcastStreamList(raw);
            parsedList.forEach((layer) => {
                layer.forEach((formats) => {
                    var _a;
                    formats.paused = !((_a = layers[formats.scid]) === null || _a === void 0 ? void 0 : _a.active);
                });
            });
            // `any` cast required because typings are broken
            this._mediaObject.simulcast.list1 = parsedList.map((simulcastFormats) => simulcastFormats.map((f) => `${f.paused ? '~' : ''}${f.scid}`).join(',')).join(';');
        }
    }
}
export class AnswerMediaSection extends MediaSection {
    constructor({ iceParameters, iceCandidates, dtlsParameters, sctpParameters, plainRtpParameters, planB = false, offerMediaObject, offerRtpParameters, answerRtpParameters, codecOptions, extmapAllowMixed = false, forceGoogConference = false }) {
            ...
            case 'video':
                {
                    ...
                    if ((this._planB || forceGoogConference) && this._mediaObject.type === 'video') {
