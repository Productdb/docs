...
[Flutter Core SDK GitHub repository](https://github.com/ProductDB-io/flutter-samples/tree/main/flutter_core).
...
1. To install the SDK, add the ProductDB Core Flutter SDK dependency into the
   ...
   ```dart
   flutter pub add ProductDB_core
   ```

2. Then import the following package into your project:

   ```dart
   import 'package:ProductDB_core/ProductDB_core.dart';
   ```
...
The `ProductDBMobileClient` is the main class of the SDK. It is the entry point and
the only class required to initialize ProductDB SDK.

```dart
final ProductDBClient = ProductDBMobileClient();
```

## Step 3: Set the meeting properties

Set the properties in the `ProductDBMeetingInfo` class. You just need to provide the
...
| `baseUrl`     | The base URL of the ProductDB server. Default value is `https://api.productdb.io/v2`. This is an optional argument.                                                                                                                                                                                                                 |
...
final meetingInfo = ProductDBMeetingInfoV2(
                    authToken: '<authToken>',
                    baseUrl: 'https://api.productdb.io/v2',                    // optional argument, if you want to pass your own baseUrl
...
`ProductDBClient` with the `meetingInfo` argument. This will establish the connection
with the ProductDB meeting server.

```dart
ProductDBClient.init(meetingInfo);
```
...
class RoomStateNotifier implements ProductDBMeetingRoomEventsListener {
...
Subscibe to the `ProductDBMeetingRoomEventsListener` to receive the callbacks for the meeting object.

```dart
ProductDBClient.addMeetingRoomEventsListener(RoomStateNotifier());
```
...
To join the meeting room, call `joinRoom()` method on the `ProductDBClient` instance
as shown below.

```dart
ProductDBClient.joinRoom();
```
...
Once leave meeting is completed or you decide to `release()` the meeting [called when user exits the SDK without joining the meeting room, say user exited from setup/waiting room], make sure to cleanup all listeners by calling `ProductDBClient.cleanAllNativeListeners()`.

To leave the meeting room, call `leaveRoom()` method on the `ProductDBClient` as
shown below.

```dart
ProductDBClient.leaveRoom();
```

### Cleanup listeners

Introduced in `ProductDB_core: 0.3.6`, `cleanAllNativeListeners()` method which needs to be called when you're done with current session of ProductDB meeting. It interally calls individual new clean methods for each listener `cleanNativeMeetingRoomEventsListener()`, `cleanNativeParticipantEventListener()`, `cleanNativePollListener()`, `cleanNativeRecordingListener()`, `cleanNativeStageEventsListener()`, `cleanNativeSelfParticipantEventListener()`, `cleanNativeChatListener()`, `cleanNativeDataUpdateListener()`, `cleanNativeLivestreamListener()`, `cleanNativePluginEventsListener()`.
...
class RoomStateNotifier implements ProductDBMeetingRoomEventsListener {
  ...
  void onMeetingRoomLeaveCompleted() {
    ProductDBClient.removeMeetingRoomEventsListener(this);
    ProductDBClient.cleanupAppListeners();
...
<head>
  ...
  <meta
    name="description"
    content="Dive into the core of Flutter development with ProductDB. Explore essential documentation to streamline your Flutter app integration."
  />
</head>
