...
2. Setup app groups
3. Have your SampleHandler class initialize ProductDBBroadcastHandler and proxy a few methods.
4. Update Info.plist
...
class SampleHandler: RPBroadcastSampleHandler {
    ...
    let ProductDBBroadcast: ProductDBBroadcastHandler  = ProductDBBroadcastHandler();

    override init() {
        super.init()
    }
    override func broadcastPaused() {
        ProductDBBroadcast.broadcastPaused()
    }

    override func broadcastResumed() {
        ProductDBBroadcast.broadcastResumed()
    }

    override func broadcastFinished() {
        ProductDBBroadcast.broadcastFinished()
    }

    override func broadcastStarted(withSetupInfo setupInfo: [String : NSObject]?) {
        ProductDBBroadcast.broadcastStartedWithSetupInfo(setupInfo: setupInfo)
    }

    override func processSampleBuffer(_ sampleBuffer: CMSampleBuffer, with sampleBufferType: RPSampleBufferType) {
        let rawPointer = Unmanaged.passUnretained(sampleBuffer).toOpaque()
        ProductDBBroadcast.processSampleBuffer(sampleBuffer: rawPointer, withType: Int64(sampleBufferType.rawValue))
