...
- To subscribe, pass the instance of `LocalUserListener` class created below
  to `addSelfEventsListener()` method of `ProductDBClient` instance as follows:

```dart {2}
ProductDBClient.addSelfEventsListener(LocalUserListener());
```

- Implement `ProductDBSelfEventsListener` to a class say `LocalUserListener`, and
  listen to event from `ProductDBSelfEventsListener` by overriding them. This class
  can be user to manage your state in the application.

```dart
class LocalUserListener implements ProductDBSelfEventsListener {
...
class LocalUserNotifier implements ProductDBSelfEventsListener {

  ...

  @override
  void onWaitListStatusUpdate(ProductDBWaitListStatus waitListStatus) {
    // ProductDBWaitListStatus is an enum with the following values:
    // if `ProductDBWaitListStatus.none`, user is not on the waitlist
    // if `ProductDBWaitListStatus.waiting`, user is on the waitlist
    // if `ProductDBWaitListStatus.accepted`, user has been accepted in the meeting.
    // if `ProductDBWaitListStatus.rejected`, user has been rejected from the meeting.
  }
...
Whenever user changes audio/video source using `setAudioDevice(ProductDBAudioDevice)` and `setVideoDevice(ProductDBVideoDevice)` respectively [ref: [here](/flutter-core/local-user/manage-media-devices)], `onAudioDevicesUpdated()` and `onVideoDevicesChanged()` are triggered respectively.
...
class LocalUserNotifier implements ProductDBSelfEventsListener {
  ...
  @override
  void onAudioDevicesUpdated() {
    // triggered on successful execution of `setAudioDevice(ProductDBAudioDevice)`
  }

  @override
  void onVideoDeviceChanged(ProductDBVideoDevice videoDevice) {
    // triggered on successful execution of `setVideoDevice(ProductDBVideoDevice)`
  }
