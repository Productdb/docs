...
```dart
final ProductDBLivestreamStatus? status = dyteClient.livestream.data.state;
```
...
LivestreamView widget returns the view for the current livestream. It takes `String? url` as an argument of the livestream which is accessible via `dyteMobileClient.livestream.url` method or `Future<String?> getUrl()` method on `ProductDBLivestream` instance, which returns `null` if there's no livestream url.
