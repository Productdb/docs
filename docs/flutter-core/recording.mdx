...
The `ProductDBClient.recording` object can be used start and stop recordings in a
meeting.

### Recording State

The `ProductDBClient.recording.recordingState` property indicates the current state of the recording. Possible states include `idle`, `recording`, `starting`, `stopping`.

### Start a recording

To start a recording, you need to call the `start()` method in the
`ProductDBClient.recording` object, as shown below.

```dart
ProductDBClient.recording.start();
```
...
```dart
ProductDBClient.recording.stop();
```

### Listening for Recording Events

To handle recording-related updates, you need to listen for
`onMeetingRecordingStateUpdated()` event. This returns `ProductDBRecordingState`
object with it.

### Listen to recording state changes

To handle recording-related updates, you need to implement `ProductDBMeetingRoomEventsListener`. This interface provides callbacks for various recording events as described in excerpt below:
...
- `onMeetingRecordingStateUpdated(ProductDBRecordingState` recordingState): Notifies when there is a change in the recording state.
...
```dart
ProductDBClient.addRecordingListener(RecordingListener());
```
...
<head>
  ...
  <meta
    name="description"
    content="Explore recording features in Flutter with ProductDB's documentation. Learn how to enable and configure recording for a seamless user experience."
  />
</head>
