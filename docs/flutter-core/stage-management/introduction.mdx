...
permissions, and kicking participants from the stage. These APIs are accessible through the `ProductDBMobileClient.stage` object.

### Stage Status

In meetings where stage management is enabled, a user's stage status can change within the values represented by the `ProductDBStageStatus`
...
The `ProductDBMobileClient.stage.status` property provides the current stage status of the local user.
...
property. This property provides a list of `ProductDBJoinedMeetingParticipant` objects whose stage status is not `onStage`.
...
```dart
ProductDBClient.stage.leave();
```

### List of Stage Events

The `ProductDBStageEventListener` interface provides callback methods for various stage events. Implement these callbacks to handle
...
class StageEventListener extends ProductDBStageEventsListener {
  ...
  void onParticipantRemovedFromStage(
      ProductDBJoinedMeetingParticipant participant) {
  ...
  @override
  void onStageStatusUpdated(ProductDBStageStatus stageStatus) {
    // Called when the local user's stage status is updated.
  }

  @override
  void onParticipantStartedPresenting(ProductDBJoinedMeetingParticipant participant) {
    // Called when a participant joins the stage.
  }

  @override
  void onParticipantStoppedPresenting(ProductDBJoinedMeetingParticipant participant) {
