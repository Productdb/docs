...
final List<ProductDBJoinedMeetingParticipant> joined = ProductDBMobileClient.participants.active;

// get active participants
final List<ProductDBJoinedMeetingParticipant> active = ProductDBMobileClient.participants.active;
```

The `ProductDBMobileClient.participants` object has the following **lists** of participants
...
Each participant in each of the `joined`, `active`, `pinned` and `screenShares` list are of type `ProductDBJoinedMeetingParticipant`, `waitlisted` list is of type `ProductDBWaitlistedParticipant`.
