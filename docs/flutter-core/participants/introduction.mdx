...
objects of `ProductDBMobileClient()` instance. To get all the objects and methods
related to participants:

```dart
  final participants = ProductDBClient.participants;
```
...
final joinedParticipants = ProductDBClient.participants.joined;
```

The `ProductDBClient.participants` object has the following properties.

- `waitlisted`: A list that contains all the participants waiting to join the
  meeting. All elements of this array are of type _ProductDBWaitlistedParticipant_.
- `joined`: A list that contains all the participants who have joined the
  meeting. All elements of this array are of type _ProductDBJoinedMeetingParticipant_.
- `active`: A list that contains all the participants except the local user who
  are supposed to be on the screen at the moment. All elements of this array are of type _ProductDBJoinedMeetingParticipant_.
- `screenshares`: A list that contains all the participants who have shared
  screen in the meeting. All elements of this array are of type _ProductDBJoinedMeetingParticipant_.
- `pinned`: Pinned participant of the meeting. It can be nullable as well. This is of type _ProductDBJoinedMeetingParticipant_.
...
- `ProductDBClient.pinned` is of type `ProductDBJoinedMeetingParticipant`.
- All participant in each of the `joined`, `waitlisted`, `active`, and
  `screenshares` list is of type `ProductDBJoinedMeetingParticipant`.

## Grid info for the room

`ProductDBGridPagesInfo` object is designed to help you with pagination decisions. It
...
ProductDBClient.broadcastMessage("type", payload);
```
...
```dart
class ParticipantNotifier implements ProductDBParticipantEventsListener{
...
<head>
  ...
  <meta name="description" content="Learn about participants in Flutter and how to integrate participant features with ProductDB's documentation. Follow our guide for a smooth introduction."/>
</head>
