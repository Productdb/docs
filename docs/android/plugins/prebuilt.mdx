...
To show the list of enabled plugins in your app, you can use the `ProductDBPluginsBottomSheet` class.

You can create an instance of this class and call the `show` method to display the list of enabled plugins as shown below:

```kotlin
val ProductDBPluginsBottomSheet = ProductDBPluginsBottomSheet()
ProductDBPluginsBottomSheet.show(
  fragmentManager,
  "SOME_TAG_HERE"
)
```
...
You can receive data from a plugin by subscribing to the `onPluginActivated` event in through our `ProductDBPluginEventsListener` interface.

```kotlin
private var pluginEventListener: ProductDBPluginEventsListener =
    object : ProductDBPluginEventsListener {
