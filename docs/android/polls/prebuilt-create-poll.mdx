...
ProductDB's Android UI Kit provides a user-friendly prebuilt screen, [ProductDBCreatePollBottomSheet](/android/components/ProductDB-poll-form), 
for creating polls in a ProductDB meeting. When using the ProductDBCreatePollBottomSheet, the poll creator can:
...
To integrate the `ProductDBCreatePollBottomSheet` into your app and allow participants to create polls, 
use the provided code snippet:

```kotlin
import productdb.io.uikit.screens.polls.ProductDBCreatePollBottomSheet

val createPollBottomSheet = ProductDBCreatePollBottomSheet()
