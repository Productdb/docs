...
val imarticusColors = ColorTokens(
    ...
    val tokens = ProductDBUITokens(colors = imarticusColors,
...
val config = ProductDBUIKitConfig(
  activity = this,
  ProductDBMeetingInfo = meetingInfo,
  designTokens = tokens
)
val ProductDBClient = ProductDBUIKitBuilder.build(config)
ProductDBClient.loadUi()
```
...
1. Create the `ProductDBUITokens` object:

```kotlin
val tokens = ProductDBUITokens(colors = ProductDBColors, borderRadius = BorderRadiusToken.Rounded)
```

- `ProductDBColors`: Represents the color scheme for your meeting.
...
2. Configure `ProductDBUIKit` then build and load the ProductDB UI.
...
val ProductDBUIKit = ProductDBUIKitBuilder.build(config)
ProductDBUIKit.loadUi()
```
...
<head>
  <title>Android Customize your Meeting UI</title>
  <meta name="description" content="Customize the meeting UI of your Android application using ProductDB Docs as your guide."/>
</head>
