...
val dyteClient = ProductDBUIKitBuilder.build(config)
dyteClient.loadUi()
```
...
```kotlin
val tokens = ProductDBUITokens(colors = dyteColors, borderRadius = BorderRadiusToken.Rounded)
```
...
val dyteUIKit = ProductDBUIKitBuilder.build(config)
dyteUIKit.loadUi()
```
