...
val joined: List<ProductDBJoinedMeetingParticipant> = meeting.participants.joined

// get all participants
val all: List<ProductDBParticipant> = meeting.participants.all
```
...
Each participant in each of the `joined`, `active`, `pinned` and `screenShares` list are of type `ProductDBJoinedMeetingParticipant`, `waitlisted` list is of type `ProductDBWaitlistedParticipant` and `all` list is of type `ProductDBParticipant`.
