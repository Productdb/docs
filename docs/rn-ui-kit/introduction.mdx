...
ProductDB's React Native UI Kit is a prebuilt design library of UI components that makes it
...
- **Use prebuilt component**: You can use prebuilt component `<ProductDBMeeting />` to
...
- ProductDB UI Kit is built using
...
- ProductDB's React Native UI Kit also provides multiple
  ...
  experience when integrating ProductDB's UI Kit in your React project. You can
  simply import the package from `@ProductDBsdk/react-native-core`, which is a hooks
  wrapper on `@ProductDBsdk/react-native-core`. For more information, see
