...
You can also subscribe to events for a single participant by implementing `ProductDBParticipantUpdateListener` callback and then passing that object to `participant.addParticipantUpdateListener(ProductDBParticipantUpdateListener)` method.
