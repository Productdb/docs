...
If you are using our UI Kits, this functionality can be handled by `ProductDBSetupFragment` or built with `ProductDBParticipantTileView` 
and `ProductDBSettingsFragment` components.
...
```kotlin
meeting.addSelfEventsListener(object : ProductDBSelfEventsListener {
