...
property. This property provides a list of `ProductDBJoinedMeetingParticipant` objects whose stage status is not `ON_STAGE`.
...
```kotlin
meeting.addStageEventsListener(object : ProductDBStageEventListener {
    ...
    override fun onPresentRequestAdded(participant: ProductDBJoinedMeetingParticipant) {
        // Called when a participant requests to join the stage. Triggered only if the local user is a host.
    }

    override fun onPresentRequestClosed(participant: ProductDBJoinedMeetingParticipant) {
    ...
    override fun onPresentRequestRejected(participant: ProductDBJoinedMeetingParticipant) {
        // Called when a participant's stage access request is denied by the host.
        // Triggered only if the local user is a host.
    }

    override fun onPresentRequestWithdrawn(participant: ProductDBJoinedMeetingParticipant) {
        // Called when a participant cancels their stage access request.
        // Triggered only if the local user is a host.
    }

    override fun onParticipantRemovedFromStage(participant: ProductDBJoinedMeetingParticipant) {
        // Called when a participant is removed from the stage by the host.
    }

    override fun onStageRequestsUpdated(accessRequests: List<ProductDBJoinedMeetingParticipant>) {
        // Called when the list of stage access requests is updated.
    }

    override fun onParticipantStartedPresenting(participant: ProductDBJoinedMeetingParticipant) {
        // Called when a participant joins the stage.
    }

    override fun onParticipantStoppedPresenting(participant: ProductDBJoinedMeetingParticipant) {
        // Called when a participant leaves the stage.
    }

    override fun onStageStatusUpdated(stageStatus: ProductDBStageStatus) {
