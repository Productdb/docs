...
You can listen to livestream events by attaching a listener by calling `addLivestreamEventsListener` on `ProductDBMobileClient` object where `ProductDBMobileClient` is an instance of `ProductDBMobileClient()`.

```kotlin
val ProductDBLiveStreamEventsListener = object : ProductDBLiveStreamEventsListener {
  ...
  override fun onLiveStreamStateUpdate(data: ProductDBLivestreamData) {
    // when there is an update in state of the livestream
  }
}
meeting.addLiveStreamEventsListener(ProductDBLiveStreamEventsListener)
```
...
```kotlin
  void onJoinRequestAccepted(ProductDBLiveStreamStagePeer peer) {}
```
...
```kotlin
  void onJoinRequestRejected(ProductDBLiveStreamStagePeer peer) {}
```
