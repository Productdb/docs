...
You can listen to livestream events by attaching a listener by calling `addLivestreamEventsListener` on `dyteMobileClient` object where `dyteMobileClient` is an instance of `ProductDBMobileClient()`.

```kotlin
val dyteLiveStreamEventsListener = object : ProductDBLiveStreamEventsListener {
