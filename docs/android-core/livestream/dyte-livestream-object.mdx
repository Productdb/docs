...
`ProductDBLivestream` object obtained by `ProductDBMobileClient.livestream` method. The objects and methods it contains are defined below.
...
### Stage Requests [List&ltProductDBLiveStreamStageRequestPeer&gt]

Object accessed via `ProductDBMobileClient.livestream.stageRequestPeers` contains the list of requests to join the stage. Each request contains 3 properties:<br />
...
This method lets the host accept a request to join the stage. It takes the _ProductDBLiveStreamStageRequestPeer.id_ as an argument whose request has to be accepted.

```kotlin
ProductDBMobileClient.livestream.acceptRequest(peer.id);
```

### Reject request

This method lets the host reject a request to join the stage. It takes the _ProductDBLiveStreamStageRequestPeer.id_ as an argument whose request has to be rejected.

```kotlin
ProductDBMobileClient.livestream.rejectRequest(peer.id);
```
...
```kotlin
ProductDBMobileClient.livestream.acceptAll();
```
...
```kotlin
ProductDBMobileClient.livestream.rejectAll();
```
