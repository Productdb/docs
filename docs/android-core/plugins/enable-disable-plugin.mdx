...
[`ProductDBPlugin`](./introduction) and exposes the following functions to enable
plugins.
...
```kotlin
val plugin: ProductDBPlugin = meeting.plugins.all[0]
...
```kotlin
val plugins: List<ProductDBPlugin> = meeting.plugins.all
...
Each plugin in `meeting.plugins` object is of type
[`ProductDBPlugin`](./introduction) and exposes the following functions to disable
plugins.
