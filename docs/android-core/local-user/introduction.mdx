...
```java {3,4}
ProductDBMeetingInfoV2 meetingInfo = ProductDBMeetingInfoV2(
      AUTH_TOKEN,
      false,
      true,
      "productdb.io"
...
```java
// get all available video devices
List<ProductDBVideoDevice> devices = meeting.localUser.getVideoDevices();
...
```java
// get all available video devices
List<ProductDBAudioDevice> devices = meeting.localUser.getAudioDevices();
