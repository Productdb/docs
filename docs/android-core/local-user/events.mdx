...
You can subscribe to various events on the local user by implementing `ProductDBSelfEventsListener` and passing the object to `meeting.addSelfEventsListener(dyteSelfEventsListener)`.
