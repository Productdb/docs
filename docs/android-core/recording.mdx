...
To handle recording-related events, implement the `ProductDBRecordingEventsListener` interface. This interface provides callbacks for 
...
- `onMeetingRecordingStateUpdated(state: ProductDBRecordingState)`: Notifies when there is a change in the recording state.
...
```kotlin
meeting.addRecordingEventsListener(object : ProductDBRecordingEventsListener {
