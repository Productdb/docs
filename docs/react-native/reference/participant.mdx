...
| isMe                 | boolean                                | Checks if the participant is the current participant, in which case it can be treated as [`ProductDBSelfParticipant`](./self-participant) |
...
| metadata.view_type   | Enum ([ProductDBViewTypes](#ProductDBviewtypes)) | The type of meeting view that the participant is in                                                                                  |
...
`ProductDBViewTypes` is an enum that enumerates the different kind of meeting views,
that are available to a participant in ProductDB meeting.
