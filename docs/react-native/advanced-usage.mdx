...
ProductDB uses VP9 codec by default, but switches to H264/VP8 depending on
participants' video codec support. VP9 is a superior codec but not all browsers
support it. If you set `autoTune` in `meetingConfig` to `false`, ProductDB will use
...
to `ProductDBMeeting`. `onError` expects a function which has a single param of type
...
type `ProductDBPlugin`, whose prototype is defined below in the reference section and
...
meeting controls. This is possible with ProductDB, with the minor caveat that you
...
`ProductDBParticipant`, whose prototype is defined below in the reference section and
can be directly used as a type if you prefer using TypeScript bindings. There
will be exactly one element of type `ProductDBSelfParticipant`, which refers to the
current participant.
...
`Array[ProductDBParticipant|ProductDBSelfParticipant]` where all meeting participants are
[`ProductDBParticipant`](./reference/participant) and the current participant is
[`ProductDBSelfParticipant`](./reference/self-participant).
...
Get unique ID for every participant. This ID is generated by ProductDB when a
participant joins the meeting. This ID is used for referring to a participant
within ProductDB, such as for
...
that participant as an instance of type `ProductDBSelfParticipant`, which is a
subclass of type `ProductDBParticipant`. This unlocks a few additional actions that
...
more about these on the [`ProductDBErrors`](./reference/productdb-errors) page in the
reference section.

```tsx
enum ProductDBErrors {
  CLIENTID_INVALID,
...
```tsx
import { ProductDBMeeting, ProductDBErrors } from "@productdb/client";
...
<ProductDBMeeting
    ...
    onError={(error) => {
        if (error = ProductDBErrors.CLIENTID_INVALID) {
