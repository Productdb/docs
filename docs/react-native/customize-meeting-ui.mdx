...
ProductDB offers a host of customizations for the UI of meetings, some of which also
...
2. call the `updateUIConfig` method on [`meeting` instance](./reference/meeting)
   and pass the [`uiConfigOptions`](./reference/ProductDB-ui-config) as a parameter
   to this call
...
[here](./reference/ProductDB-ui-config).
...
ProductDB allows you to specify four color values.
...
The default behaviour of a meeting on ProductDB is to take up the full viewport.
