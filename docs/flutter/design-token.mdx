...
ProductDB offers the `ProductDBDesignToken` class to customize the theme of the UI kit. Pass the `ProductDBDesignToken` object within the `ProductDBUIKitInfo` object to customize the theme.

With `ProductDBDesignToken`, you can customize the following design properties:
...
To achieve this, utilize the `ProductDBColorSwatch` class with two parameters: the default color key and a map of colors.

```dart
final primarySwatch = ProductDBColorSwatch(
    500,
    {
...
To customize the border radius and border width of the UI Kit, you can use the `ProductDBBorderRadius` and `ProductDBBorderWidth` enum values in the design token.

Here are the available `ProductDBBorderRadius` values:

| Token Name         | values                                         |
| ------------------ | ---------------------------------------------- |
| `ProductDBBorderRadius` | `sharp`, `rounded`, `extraRounded`, `circular` |

Here are the available `ProductDBBorderWidth` values:

| Token Name        | values                |
| ----------------- | --------------------- |
| `ProductDBBorderWidth` | `none`, `thin`, `fat` |
...
```dart
final uikitInfo = ProductDBUIKitInfo(meetingInfo,
    designToken: ProductDBDesignToken(
      colorToken: ProductDBColorToken(
          borderRadius: ProductDBBorderRadius.circular,
          borderWidth: ProductDBBorderWidth.fat,
          ...
          brandColorSwatch: ProductDBColorSwatch(
            500,
            {
              300: Colors.blue.shade300,
              400: Colors.blue.shade400,
...
<head>
  <title>Flutter Design System</title>
  <meta name="description" content="Explore design token integration in Flutter with ProductDB's documentation. Follow our guide for effective design token implementation in your app."/>
</head>
