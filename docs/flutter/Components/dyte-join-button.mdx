...
Here is a basic example of how to use the `ProductDBJoinButton` in your Flutter app:
...
- `ProductDBMobileClient`: (**Required**) An instance of `ProductDBMobileClient` to manage meeting room events.
- `onMeetingJoined`: (Optional) A callback that gets called when the meeting room is successfully joined.
- `ProductDBDesignToken`: (Optional) An instance of `ProductDBDesignTokens` for customizing the button's appearance according to the ProductDB Design System.
