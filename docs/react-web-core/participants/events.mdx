...
```ts
const viewMode = useProductDBSelector((meeting) => meeting.participants.viewMode);
```

### Page change

```ts
const pageChanged = useProductDBSelector(
...
```ts
const activeSpeaker = useProductDBSelector(
...
```ts
const pinnedParticipants = useProductDBSelector(
...
```ts
const videoEnabled = useProductDBSelector(
...
```ts
const audioEnabled = useProductDBSelector((m) =>
...
```ts
const screensharingParticipant = useProductDBSelector((m) =>
