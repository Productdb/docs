...
```ts
const meetingconnected = useProductDBSelector(
...
const meetingTitle = useProductDBSelector((meeting) => meeting.meta.meetingTitle);
useEffect(() => {
  console.log('I got meetingTitle as: ', meetingTitle);
}, [meetingTitle]);

const meetingStartedTimestamp = useProductDBSelector(
