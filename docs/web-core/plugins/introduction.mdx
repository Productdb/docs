...
collaborative. ProductDB provides a bunch of inbuilt plugins to choose from, you can
...
Each plugin in the map is of type `ProductDBPlugin`.

```ts
interface ProductDBPlugin {
  baseURL: string;
  createdAt: string;
  description: string;
  id: string;
