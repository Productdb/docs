...
1. Initialize the ProductDB client.
...
To join the meeting room, call join() method on the ProductDBClient instance as
shown below.
...
To leave the meeting room, call `leaveRoom()` method on the ProductDBClient as shown
below.
