...
[sample code](https://github.com/ProductDB-io/flutter-samples/tree/main/flutter_uikit) for Flutter. You can clone
and run a sample application from the
[Flutter Samples GitHub repository](https://github.com/ProductDB-io/flutter-samples).
...
```dart
final meetingInfoV2 = ProductDBMeetingInfoV2(authToken: '<auth_token>');
```
...
```dart
/* Passing the ProductDBMeetingInfoV2 object `meetingInfoV2` you created in the Step 3,
   and your context in `clientContext` parameter. */

final ProductDBUIKit uiKit = ProductDBUIKit(
...
To launch the meeting UI all you need to do is call the `loadUI()` method of the `ProductDBUIKit` object which will return a `Widget`. You can push this widget as a page to start the flow of prebuilt Flutter UI Kit.
