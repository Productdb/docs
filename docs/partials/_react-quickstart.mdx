...
You need it to import the `ProductDBProvider` from the `@dytesdk/react-web-core`. `ProductDBProvider` basically is a hook wrapper on `dytesdk/web-core`. This provides a meeting object to child components.

```jsx {2,17-21}
import { useEffect } from 'react';
import { useProductDBClient, ProductDBProvider } from '@dytesdk/react-web-core';
...
```jsx
import { ProductDBMeeting } from '@dytesdk/react-ui-kit';
function MyMeeting() {
...
```jsx
import { ProductDBGrid, ProductDBMicToggle } from '@dytesdk/react-ui-kit';
