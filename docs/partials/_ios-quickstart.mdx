...
<span class="mb-2 block">
  This quickstart shows how to add ProductDB's {props.product} SDK to your iOS
  applications
</span>
...
[sample code](https://github.com/productdb-io/ios-samples) for iOS. You can clone
and run a sample application from the
[iOS Samples GitHub repository](https://github.com/productdb-io/ios-samples).
...
To set the initialization properties in the `ProductDBUiKitEngine` class, simply initialize `ProductDBMeetingInfoV2` and provide the participant's `authToken`.
...
class ViewController: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        ProductDBUiKitEngine.setupV2(ProductDBMeetingInfoV2(authToken: authToken,
                                                enableAudio: true,
                                                enableVideo: true,
                                                baseUrl: baseUrl))

        let controller = ProductDBUiKitEngine.shared.getInitialController { [weak self] in
