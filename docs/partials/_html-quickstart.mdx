...
2. Call the `DyteClient.init()` method from the `web-core` package and pass the `authToken`. This will establish the connection with the ProductDB server. You should keep the returned `meeting` object.
