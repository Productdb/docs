...
import { useDyteClient, ProductDBProvider } from '@dytesdk/react-native-core';
  ...
  if (meeting)
    return (
      <ProductDBProvider value={meeting}>
        ...
        {/* Components rendered inside ProductDBProvider can access ProductDBClient object using useDyteClient() hook */}
      </ProductDBProvider>
    );
}
```
