...
- **Possible solution**: The `joinRoom()` method is used to join a meeting room in ProductDB. Try calling this method on the `DyteClient`:
...
- **Possible reason**: Indicates a problem with the `DyteClient`.
- **Possible solution**: The `leaveRoom()` method is used to leave a meeting room in ProductDB. Try calling this method on the `DyteClient`:
