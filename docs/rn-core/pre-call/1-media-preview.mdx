...
This section focuses on pre-call functionality, providing developers with the tools needed to prepare the media environment before joining the meeting. If you are using our UI Kits, this will be handled by `ProductDBSetupScreen` or could be built with `ProductDBParticipantTile` and other components.
...
```ts
const audioEnabled = useProductDBSelector((m) => m.self.audioEnabled);
const audioTrack = useProductDBSelector((m) => m.self.audioTrack);
```
...
```ts
const videoEnabled = useProductDBSelector((m) => m.self.videoEnabled);
const videoTrack = useProductDBSelector((m) => m.self.videoTrack);
```
