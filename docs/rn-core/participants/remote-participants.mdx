...
const joinedParticipants = useProductDBSelector(meeting.participants.joined);
```
...
maps is of type [`ProductDBParticipant`](../reference/ProductDBParticipant.md). Read more
about each individual `participant` object
[here](../participants/participant-object.mdx).

Each of these maps are of type
[`ProductDBParticipantMap`](../reference/ProductDBParticipantMap.md), and therefore emit a
...
<head>
  <title>React Web Core Participants</title>
</head>
