...
Instead of a traditional publish-subscribe model, where a user can publish their media and others can choose to subscribe, ProductDB comes with an optional managed configuration.
...
Using ProductDB's stage management APIs a user can do actions such as leave and join stage, manage stage requests and permissions, kick participants and so on.

## Access the stage APIs

The stage module can be accessed under [`meeting.stage`](/react-web-core/reference/ProductDBStage) namespace.
...
ProductDB's stage management APIs allow participants to receive and manage stage requests as well as leave and join the stage.
