...
```html
<script>
  window.addEventListener('ProductDB.deviceEmulatorLoaded', () => {
    // use device emulator methods here...
  });
</script>
<script src="https://cdn.jsdelivr.net/npm/@ProductDBsdk/device-emulator/dist/index.iife.js"></script>
```
...
```bash
npm install @ProductDBsdk/device-emulator
```

use the package like below

```js
import '@ProductDBsdk/device-emulator';
...
```bash
yarn add @ProductDBsdk/device-emulator
```
...
<head>
  ...
  <meta name="description" content="Learn about ProductDB's community package for device emulation. Explore the documentation to emulate various devices and test applications effectively."/>
</head>
