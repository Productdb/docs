...
available in both [Swift](https://github.com/ProductDB-io/ios-samples/tree/main/iOS-core) and [SwiftUI](https://github.com/ProductDB-io/ios-samples/tree/main/ProductDBSwiftUI-Core).
...
- [Configure a ProductDB meeting](#step-3-configure-a-ProductDB-meeting)
- [Initialize the ProductDB meeting](#step-4-initialize-the-ProductDB-meeting)
- [Go live with your ProductDB meeting](#step-5-go-live-with-your-ProductDB-meeting)
...
2. Add 'ProductDBiOSCore' to your Podfile.

<CocoaPodInstallation pkg="ProductDBiOSCore" path="c/d/1/ProductDBiOSCore" />
...
Add `ProductDBiOSCore` SDK through Swift Package Manager in Xcode. Use https://github.com/ProductDB-in/ProductDBMobileCoreiOS.git as the package source.
...
1. The `ProductDBMobileClient` is the main class of the SDK. It is the main entry
   point of the SDK. It is the only class that you need to instantiate in order
   to use the SDK. To instantiate ProductDBMobileClient, you should use
   `ProductDBiOSClientBuilder().build()`.

```swift
let meeting = ProductDBiOSClientBuilder().build()
```
...
Add `authToken` that you got from the REST API to constructor of ProductDBMeetingInfoV2 - [Add Participant API](/api#/operations/addParticipant)
...
| `baseUrl`     | Base URL of the ProductDB's enviorment you have created the meeting on.                                                                                                                      |

```swift
let meetingInfo = ProductDBMeetingInfoV2(authToken: authToken,
                                  enableAudio: true,
                                  enableVideo: true,
...
```swift
meeting.doInit(ProductDBMeetingInfo_: meetingInfo)
```
...
) to know its completion state, success or failure, Listen to below callbacks of 'ProductDBMeetingRoomEventsListener'.
...
```swift
meeting.doInit(ProductDBMeetingInfo: ProductDBMeetingInfoV2,
...
```swift
 meeting.doInit(ProductDBMeetingInfo: meetingInfo) {
...
```swift
extension MeetingViewModel: ProductDBSelfEventsListener {
...
<head>
  ...
  <meta
    name="description"
    content="Dive into ProductDB's core documentation for iOS. Follow our guide for comprehensive understanding and effective iOS core development."
  />
</head>
