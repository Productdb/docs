...
`ProductDBLivestream` object obtained by `meeting.livestream` method. The objects and methods it contains are defined below.
...
### stage requests [*_List&ltProductDBLiveStreamStageRequestPeer&gt_*]
...
You can listen to livestream events by attaching a listener by calling `addLivestreamEventsListener` on `meeting` object where `meeting` is an instance of `ProductDBMobileClient()`.
...
```swift
extension LivestreamViewController: ProductDBLiveStreamEventsListener {
    ...
    public func onLiveStreamStateUpdate(data: ProductDBLivestreamData) {
