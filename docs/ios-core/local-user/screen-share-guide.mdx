...
2. Setup app groups.
3. Download `ProductDBSampleHandler` class from [Github repo](https://github.com/ProductDB-io/iOS-ScreenShare) .
4. Have your SampleHandler class inherited from this downloaded class `ProductDBSampleHandler` .
5. Update Info.plist.
...
class SampleHandler: ProductDBSampleHandler {}
```
...
```swift
ProductDBClient.localUser.disbaleScreenshare()
```
