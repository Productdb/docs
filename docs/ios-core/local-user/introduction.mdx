...
```swift
let meetingInfo = ProductDBMeetingInfo(
...
```swift
meeting.localUser.setVideoDevice(videoDevice: ProductDBVideoDevice)
```
...
<head>
  ...
  <meta
    name="description"
    content="Learn about local user management in iOS Core with ProductDB Docs. Understand the fundamentals for effective integration and app customization."
  />
</head>
