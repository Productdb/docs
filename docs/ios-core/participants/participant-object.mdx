...
`onRoomMessage` method from callback `ProductDBSelfEventsListener`. You can
subscribe to this events by calling
`meeting.addChatEventsListener(ProductDBSelfEventsListener)`

[check this ProductDBSelfEventsListener broadcastMessage documentation](/ios-core/local-user/events#listen-to-broadcast-message-within-the-room)
