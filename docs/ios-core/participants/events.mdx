...
You can also subscribe to events for a single participant by implementing `ProductDBParticipantUpdateListener` protocol and then passing that object to `participant.addParticipantUpdateListener(dyteParticipantUpdateListener)` method.
