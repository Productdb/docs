...
You can use the provided `<ProductDB-meeting />` monolith component in case you want
...
<div id="app">
  ...
  <div id="joined-screen" style="display: none;">
    <ProductDB-grid id="my-grid" class="ProductDB"></ProductDB-grid>
    <ProductDB-mic-toggle class="ProductDB"></ProductDB-mic-toggle>
    <ProductDB-camera-toggle class="ProductDB"></ProductDB-camera-toggle>
  </div>
</div>
...
<script>
  ...
  function passMeeting() {
    const els = document.getElementsByClassName('ProductDB');
    for (const el of els) {
      el.meeting = meeting;
    }
  }
...
| Prop       | Description                                                                      |
| :--------- | :------------------------------------------------------------------------------- |
| `meeting`  | The web-core `ProductDBClient` object. Contains all the meeting specific data & APIs. |
| `config`   | The [`UI Config`](../reference/interfaces/UIConfig) config                       |
| `t`        | [i18n helper](../reference/#ProductDBi18n) method                                     |
...
```js
const gridEl = document.createElement('ProductDB-grid');
...
```html
<ProductDB-button>Primary Button (Default)</ProductDB-button>
<ProductDB-button variant="secondary">Secondary button</ProductDB-button>
<ProductDB-button variant="danger">Danger button</ProductDB-button>
<ProductDB-button variant="ghost">Ghost button</ProductDB-button>
```
...
```js
const nameTagSelf = document.createElement('ProductDB-grid');
...
- Parent Grid component - `ProductDB-grid`
- Normal Grid components - `ProductDB-simple-grid`, `ProductDB-mixed-grid`,
...
<ProductDB-grid aspect-ratio="16:9" gap="8" size="sm"></ProductDB-grid>
```
...
const simpleGrid = document.createElement('ProductDB-simple-grid');
...
<head>
  <title>Angular UI Kit Components Basics</title>
  <meta name="description" content="Learn the basics of components in ProductDB's Angular UI Kit through our detailed documentation."/>
</head>
