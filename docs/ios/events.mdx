...
`ProductDBSelfEventsListener` and passing the object to
`meeting.addSelfEventsListener(ProductDBSelfEventsListener)`.
...
`ProductDBMeetingRoomEventsListener`. You can attach this observer by calling
`meeting.addMeetingRoomEventsListener(listener)`.

```kotlin
meeting.addMeetingRoomEventsListener(object : ProductDBMeetingRoomEventsListener {
    ...
    override fun onMeetingRecordingStateUpdated(state: ProductDBRecordingState) {
...
<head>
  ...
  <meta
    name="description"
    content="Access comprehensive documentation on iOS events with ProductDB Docs. Stay informed and integrate events seamlessly into your app."
  />
</head>
