...
2. `AppTheme` is a singleton class that relies on the `AppThemeProtocol` to set corner radius and border size for various components used within `ProductDBUiKit`. You can customize the `AppTheme` by setting any object that confirms to the `AppThemeProtocol`.
