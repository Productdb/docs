...
```swift
extension ChatViewController: ProductDBChatEventsListener {
...
```swift
var messages: [ProductDBChatMessage]?
let messageTableView = UITableView()
let sendFileButton = ProductDBButton(style: .iconOnly(icon: ProductDBImage(image: sendFileImageIcon)), ProductDBButtonState: .focus)
let sendImageButton = ProductDBButton(style: .iconOnly(icon: ProductDBImage(image: sendImageIcon)), ProductDBButtonState: .active)
let sendMessageButton = ProductDBButton(style: .iconOnly(icon: ProductDBImage(image: sendMessageIcon)), ProductDBButtonState: .active)
...
`ProductDBChatMessage` can be of type `ProductDBTextMessage`, `ProductDBFileMessage` or `ProductDBImageMessage`.
