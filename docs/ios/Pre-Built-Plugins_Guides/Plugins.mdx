...
We have to use a class [ProductDBPluginViewController](/ios/components/productdb-plugins), which is used to show exising list of plugins
...
```swift
 private func showPluginScreen() {
  let controller = ProductDBPluginViewController(plugins: meeting.plugins.all)
