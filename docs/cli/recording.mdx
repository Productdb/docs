...
```shell
ProductDB recordings start <meeting id>
```
...
```shell
ProductDB recordings stop <meeting id>
```
...
```shell
ProductDB recordings get <meeting id> <recording id>
```

:::tip The `ProductDB recordings stop` command will helpfully output the complete
command you need to retrieve that particular recording! :::

<head>
  <title>CLI Recordings</title>
  <meta name="description" content="Understand how to enable and configure recording features using ProductDB CLI. Capture and review meeting content seamlessly."/>
</head>
