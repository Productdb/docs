...
ProductDB's default header component `ProductDBHeader` can be used as the following.

```tsx
<ProductDBHeader meeting={meeting} />
```

Following code shows how you can customise the ProductDBHeader or build it from scratch as per your use case.
...
file={`
import { defaultConfig, generateConfig, ProductDBLogo, ProductDBRecordingIndicator, ProductDBLivestreamIndicator, ProductDBMeetingTitle, ProductDBGridPagination, ProductDBParticipantCount, ProductDBViewerCount, ProductDBClock } from '@ProductDBsdk/react-ui-kit';
...
function HeaderWithCustomUI({
    ...
    return <div className='flex justify-between w-full bg-black text-white'>
        <div id="header-left" className="flex items-center h-[48px]">
            <ProductDBLogo meeting={meeting} />
            <ProductDBRecordingIndicator meeting={meeting}/>
            <ProductDBLivestreamIndicator meeting={meeting}/>
        </div>
        ...
        <div id="header-right" className="flex items-center h-[48px]">
            <ProductDBGridPagination meeting={meeting} states={states}/>
...
return (
  /**
  * Using a ref hack, we are adding "ProductDBStateUpdate" listener,
...
function setFullScreenToggleTargetElement({config, targetElementId}: { config: UIConfig, targetElementId: string }){
    ...
    ['ProductDB-more-toggle.activeMoreMenu', 'ProductDB-more-toggle.activeMoreMenu.md', 'ProductDB-more-toggle.activeMoreMenu.sm'].forEach((configElemKey) => {
        ...
        configElem?.forEach((ProductDBElemConfigSet: any) => {
            if (ProductDBElemConfigSet[0] === 'ProductDB-fullscreen-toggle') {
                ProductDBElemConfigSet[1].targetElement = document.querySelector("#"+targetElementId);
            }
        });
    });
}
...
Please note that the `ProductDBRecordingIndicator` will be shown only when recording is in-progress. Similarly `ProductDBLivestreamIndicator` only shows "Live" indicator if the preset is a livestream preset.

if user's preset has a logo, that logo will be shown using `ProductDBLogo` component.
