...
file={`
import { defaultConfig, generateConfig } from '@ProductDBsdk/react-ui-kit';
import { useProductDBMeeting, useProductDBSelector } from '@ProductDBsdk/react-web-core';
import { useEffect, useState } from 'react';

function CustomProductDBMeeting({ meeting, config, states, setStates }: { meeting: ProductDBClient, config: UIConfig,  states: CustomStates, setStates: SetStates}){
    if (!meeting) {
...
return (
  ...
  */
  <div className="flex w-full h-full bg-black text-white justify-center items-center" ref={(el) => {
      ...
      <CustomProductDBMeeting meeting={meeting} config={config} states={states} setStates={setStates} />
  </div>
);
...
In the next steps, we will learn how we can create custom header, footer and the stage UI using ProductDB components.