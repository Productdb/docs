...
import type ProductDBClient from '@dytesdk/web-core';
...
export default function CustomMeetingPreview() {
  ...
  const permissions = useProductDBSelector((m) => m.self.permissions);
  ...
  useEffect(() => {
    ...
    const videoBackground = new VideoBackground({
      modes: ['blur', 'virtual'],
      images: [
        'https://assets.productdb.io/backgrounds/bg_0.jpg',
        'https://assets.productdb.io/backgrounds/bg_1.jpg',
        'https://assets.productdb.io/backgrounds/bg_2.jpg',
        'https://assets.productdb.io/backgrounds/bg_3.jpg',
        'https://assets.productdb.io/backgrounds/bg_4.jpg',
      ],
      randomCount: 8,
      selector: '#productdb-integration-wrapper',
    });
...
<ProductDBControlbarButton
  ...
  icon={effectsSVG}
  onClick={() => {
    const changer = document.querySelector('productdb-background-changer');
    if (changer) {
      const isOpen = changer?.getAttribute('data-open') === 'true';
      changer.setAttribute('data-open', isOpen ? 'false' : 'true');
      if (changer.shadowRoot?.querySelector('#main')) {
