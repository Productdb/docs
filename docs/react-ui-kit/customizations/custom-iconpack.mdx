...
2. Initialize the ProductDB client using the `useProductDBClient()` hook. It returns an array with two values, the meeting object and a function to initialize the meeting.
...
5. Initialize the meeting using the `initMeeting()` method exposed by `useProductDBClient`.
6. Pass the meeting object to the UI Kit.

The `ProductDBMeeting` component generates the default UI experience. Read more about how to customize the UI [here](/react-ui-kit/basics/integrate).
...
ProductDB's default icon set is available at [icons.productdb.io](https://icons.productdb.io/). You can modify and generate your custom icon set from here.

In order to replace ProductDB's default icon set with your own, you can pass the link to your icon set in the `ProductDBMeeting` component like so.

```jsx
 <ProductDBMeeting meeting={meeting!} iconPackUrl="my-icon-pack.json" />;
```
...
<head>
  <title>React UI Kit Customize Meeting Icons</title>
    <meta name="description" content="Learn how to customize meeting icons in React using ProductDB's features. Follow our guide for effective icon customization and integration."/>
</head>
