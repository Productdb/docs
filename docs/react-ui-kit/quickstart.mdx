---
sidebar_position: 2
---

# Quickstart

This quickstart shows how to use ProductDB's UI Kit prebuilt components to add live
video and audio to your React application with minimal coding and a variety of
meeting UI customization options.

For getting started quickly, you can use our
[sample code](https://github.com/ProductDB-io/react-samples). You can clone and run a
sample application from the
[React UI Kit GitHub repository](https://github.com/ProductDB-io/react-samples).

## Before Getting Started

- Make sure you've read the [Getting Started with ProductDB](/getting-started) topic
  and completed the following steps:

  - Create a [ProductDB Developer Account](https://dev.productdb.io/)
  - Create a [ProductDB Meeting](/api/?v=v2#/operations/create_meeting)
  - [Add Participant](/api/?v=v2#/operations/add_participant) to the meeting

## Step 1: Install the SDK

Since the UI Kit is built on top of the Core SDK, you must install the
`react-web-core` package along with the `ui-kit`.

`react-web-core` consists of hooks written on top of `web-core` which makes it
easy to use [`web-core`](https://npmjs.com/package/@ProductDBsdk/web-core) in React
applications.

You can install the package using npm or Yarn.

```bash npm2yarn
npm install @ProductDBsdk/react-ui-kit @ProductDBsdk/react-web-core
```

:::tip Import Errors

If you get errors when importing the `react-ui-kit` and `react-web-core`
packages, try installing them separately.

:::

### Version

|                         |                                                                                                                              |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| @ProductDBsdk/react-ui-kit   | [![npm version](https://badge.fury.io/js/@ProductDBsdk%2Freact-ui-kit.svg)](https://badge.fury.io/js/@ProductDBsdk%2Freact-ui-kit)     |
| @ProductDBsdk/react-web-core | [![npm version](https://badge.fury.io/js/@ProductDBsdk%2Freact-web-core.svg)](https://badge.fury.io/js/@ProductDBsdk%2Freact-web-core) |

## Step 2: Initialise meeting

Here's a series of steps that you need to perform:

1. Set up `ProductDBProvider`. You need it to import the `ProductDBProvider` from the
   `ProductDBsdk/react-web-core`. ProductDBProvider basically is a hook wrapper on
   ProductDBsdk/web-core. This provides a meeting object to child components.
2. Initialize the ProductDB client. Use the `useProductDBClient()` hook and `initMeeting`
   to initialize a client.
3. Call the `init()` method and pass the `authToken`:

|             |                                                                                                                                                                                   |
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `authToken` | After you've created the meeting, add each participant to the meeting using the [Add Participant API](/api#/operations/add_participant). The API response contains the authToken. |

4. Pass the meeting object to UI Kit, which will use it to retrieve meeting
   information and display it on the user interface.

   The meeting object serves as the link between web-core and UI Kit, allowing
   them to communicate with one another. Once the UI Kit has the meeting object,
   it can join and leave meetings, and so on.

```jsx
import { useEffect } from 'react';
import { useProductDBClient } from '@ProductDBsdk/react-web-core';
import { ProductDBMeeting } from '@ProductDBsdk/react-ui-kit';

export default function App() {
  const [meeting, initMeeting] = useProductDBClient();

  useEffect(() => {
    initMeeting({
      authToken: '<auth-token>',
      defaults: {
        audio: false,
        video: false,
      },
    });
  }, []);

  return (
    <ProductDBProvider value={meeting}>
      <MyMeetingUI />
    </ProductDBProvider>
  );
}
```

## Step 3: Use `ProductDBMeeting` component

In the following example, a meeting is created using the `useProductDBMeeting`
component. `useProductDBMeeting` essentially returns the meeting object you passed to
the `ProductDBProvider`.

`ProductDBMeeting` renders the entire meeting UI. It loads your preset and renders
the UI based on it. With this component, you don't have to handle all the
states, dialogs, and other smaller bits of managing the application.

For more information on the other props of `ProductDBMeeting`, see
[ProductDBMeeting](/react-ui-kit/components/ProductDB-meeting).

import CodeRunner from '@site/src/components/CodeRunner/CodeRunner';

<CodeRunner 
highlight={[{start: 9, end: 9}]}
hide={[{start:1, end: 3}]}
file={`
import { ProductDBMeeting } from '@ProductDBsdk/react-ui-kit';
import { useProductDBMeeting } from '@ProductDBsdk/react-web-core';

export default function MyMeetingUI() {
  const { meeting } = useProductDBMeeting();

return (
<ProductDBMeeting mode="fill" meeting={meeting} showSetupScreen={false} />
);
}
`} />

<head>
  <title>React UI Kit Quickstart</title>
</head>
