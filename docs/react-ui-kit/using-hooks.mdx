...
`ProductDBsdk/react-web-core`, which is a hooks wrapper on `ProductDBsdk/web-core`.
...
```jsx
import { useProductDBSelector, useProductDBMeeting } from '@ProductDBsdk/';
...
import { ProductDBGrid, ProductDBButton } from '@ProductDBsdk/react-ui-kit';
