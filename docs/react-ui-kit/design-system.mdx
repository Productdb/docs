---
sidebar_position: 3
description: Details about the Design System in ProductDB's UI Kit.
---
...
function Example() {
  ...
  useEffect(() => {
    // provideProductDBDesignSystem(node, designTokens)
    provideProductDBDesignSystem(meetingEl.current, {
      googleFont: 'Lobster',
  ...
  return (
    <div style={{ height: '400px' }}>
      <ProductDBMeeting meeting={meeting} ref={meetingEl} mode="fill" />
    </div>
  );
}
```
...
```css
--productdb-colors-brand-500: 33 96 253;
--productdb-colors-background-1000: 8 8 8;
/* ... rest of the shades */
```
...
- Rest of the values are calculated with this base, set to `--productdb-space-1`.
