...
- The `@ProductDBsdk/opentok-shim` package serves as a wrapper over the ProductDB SDK with an external API that is fully compatible with Opentok JS. As a drop-in replacement for Opentok JS, it can handle most of the methods and events that are present in the original library.
...
```bash npm
npm install @ProductDBsdk/opentok-shim
```

2. Once you have installed the package, you can use it in your project by importing it using the following statement:

```bash
import * as OT from "@ProductDBsdk/opentok-shim";
```

## Usage

The `@ProductDBsdk/opentok-shim` shares the same API as the OpenTok video SDK. You can use most of the methods that are supported by it.
