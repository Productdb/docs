...
```tsx
<productdb-participant-tile participant={meeting.self} />
```
...
But you don't need this event to render remote participants, the `productdb-participant-tile` component sets up this listener on its own and will render the video on its own

```tsx
<productdb-participant-tile participant={remoteParticipant} />
```

It is even easier in React, here is how to render every participant

```tsx
const participants = useProductDBSelector((m) => m.participants.active);

return (<div>
      {
          participants
             .map((p) => (<ProductDBParticipantTile participant={p} />)
      }
   </div>);
```
