...
- In ProductDB you create a meeting by calling a similar REST API
  https://productdb.dev/api#/operations/create_meeting
...
```html
<script src="https://cdn.ProductDB.in/core/ProductDB.js" />
```
...
```js
import ProductDBClient from '@ProductDBsdk/web-core';

meeting = await ProductDBClient.init({ authToken: TOKEN });
await meeting.join();
```
