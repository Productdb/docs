...
You can access all of our v2 API endpoints at `https://api.productdb.io/v2/`.
...
1.  First, register on our [Developer Portal](https://dev.productdb.io).
...
[Create presets](https://dev.productdb.io/roles-presets) using the Developer portal.
...
```bash
curl --request POST \
  --url https://api.productdb.io/v2/meetings \
...
```bash
curl --request POST \
  --url https://api.productdb.io/v2/meetings/<meetingId>/participants \
  --header 'Authorization: Basic ZGV2QGR5dGUuaW46MTIzNDU2Nzg5YXNkaGZnaGo=' \
  --header 'Content-Type: application/json' \
  --data '{
  "name": "Mary Sue",
...
```bash
curl --request POST \
  --url https://api.productdb.io/v2/recordings \
...
```bash
curl --request PUT \
  --url https://api.staging.ProductDB.in/v2/recordings/<recordingId> \
...
```bash
curl --request POST \
  --url https://api.productdb.io/v2/webhooks \
  --header 'Authorization: Basic NzBjNWQzOTEtNGJhYy00Y2YzLTk5MDctYmVjMjA1Nzk4YWRiOmQyNzBmYjJmOGNiNGUzZWY1MGI1' \
  --header 'Content-Type: application/json' \
  --data '{
  "name": "Attendance",
...
<head>
  ...
  <meta
    name="description"
    content="Get started with ProductDB's REST APIs. Follow our guide for a quickstart on using REST APIs for seamless integration and development."
  />
</head>
