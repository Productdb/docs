...
// Get current status
ProductDBClient.localUser.screenShareEnabled;
```
...
const screenshareEnabled = useProductDBSelector((m) => m.self.screenShareEnabled);
...
```jsx
import { ProductDBScreenShareToggle } from '@ProductDBsdk/react-ui-kit';
...
```jsx
import { ProductDBScreenShareToggle } from '@ProductDBsdk/react-native-ui-kit';
...
```js
meeting = await ProductDBClient.init({
  authToken,
  defaults: {
    screenShare: {
