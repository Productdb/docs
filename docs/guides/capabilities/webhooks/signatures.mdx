...
signature in the productdb-signature header of each event. This allows you to confirm
...
`https://api.productdb.io/.well-known/webhooks.json`. You can obtain it by
making a GET request to this endpoint.

```
curl -X GET "https://api.productdb.io/.well-known/webhooks.json"
```

### Check for the signature header

Every incoming request will have a custom `productdb-signature` header. This is the
...
app.post('/webhook', express.json({ type: 'application/json' }), (req, res) => {
  const signature = req.headers['productdb-signature'];
...
<head>
  <title>Verify Webhooks Using ProductDB's Signature and Webhook ID Guide</title>
  <meta name="description" content="Learn how to use signatures with ProductDB's webhook capabilities. Follow our guide for effective signature implementation."/>
</head>
