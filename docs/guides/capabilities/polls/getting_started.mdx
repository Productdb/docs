...
The meetings polls object can be accessed using `ProductDBClient.polls`. `ProductDBClient.polls.polls` returns an array of all polls created in a meeting, where each element is an object of type ProductDBPollMessage.

The `ProductDBPollMessage` has the following properties:
...
| options   | Array of ProductDBPollOption object, contains all the options to the poll question |

The `ProductDBPollOption` has the following properties:

| Property | Description                                                                  |
| -------- | ---------------------------------------------------------------------------- |
| text     | Contains the option text                                                     |
| votes    | List of ProductDBPollVote object, which contains info about voters of this option |
| count    | Int of number of votes given to this option                                  |

The `ProductDBPollVote` has the following properties:
...
`meeting.polls.polls` returns an array of all polls created in a meeting, where each element is an object of type `ProductDBPollMessage`

The type `ProductDBPollMessage` is the main class for any poll in ProductDB. It also contains list of `ProductDBPollOption` which are options for a given poll. And every `ProductDBPollOption` has list of votes inside of it. Votes are objects of class ProductDBPollVote which internally has id and name of the vote.
...
```bash
curl --request POST \
  --url https://api.productdb.io/v2/meetings/meeting_id/active-session/poll \
...
```dart
final poll = ProductDBClient.polls.polls[0];
final selectedPollOption = poll.options[0];
ProductDBClient.polls.vote(poll, selectedPollOption);
```
...
To be able to receive new poll messages you need to implement a method onPollUpdates() method from callback `ProductDBPollEventsListener`:
...
```dart
ProductDBClient.addPollEventsListener(PollEventsListeners());
```
...
To receive new poll messages, you must implement the` onPollUpdates()` method from the callback `ProductDBPollEventsListener`. You can subscribe to this event by using the meeting.`addMeetingEventsListener(ProductDBPollEventsListener)` method.

```swift
extension MeetingViewModel: ProductDBPollEventsListener {
