...
The meetings polls object can be accessed using `dyteClient.polls`. `dyteClient.polls.polls` returns an array of all polls created in a meeting, where each element is an object of type ProductDBPollMessage.
...
To receive new poll messages, you must implement the` onPollUpdates()` method from the callback `ProductDBPollEventsListener`. You can subscribe to this event by using the meeting.`addMeetingEventsListener(dytePollEventsListener)` method.
