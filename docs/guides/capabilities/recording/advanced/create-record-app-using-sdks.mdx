...
The custom recording SDKs are used on top of the [UI kit](/react-ui-kit) or [Vanilla JS](/ui-kit). The `ProductDBRecording` class is used for managing the recording functionality within the SDK.

## Constructor

`constructor(options)`

Creates an instance of the `ProductDBRecording` class.
...
```js
init(client: ProductDBClient)
```

Initiates the SDK by providing a `ProductDBClient` object. Call this after creating the meeting object and before calling `meeting.joinRoom()`.
...
```js
npm i @ProductDBsdk/recording-sdk
```

### Step 2: Import the `ProductDBRecording` object

```js
import { ProductDBRecording } from '@ProductDBsdk/recording-sdk';
```

### Step 3: Create the `ProductDBRecording` object

```js
const recordingSdk = new ProductDBRecording(options);
```
...
<head>
  ...
  <meta name="description" content="Learn how to create a recording app using ProductDB's SDKs. Follow our guide for effective app creation and integration."/>
</head>
