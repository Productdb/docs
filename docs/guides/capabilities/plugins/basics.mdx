...
```dart
List<ProductDBPlugin> plugins = await dyteClient.plugins.all;
```
...
Any plugin activation / deactivation will trigger a callback. To be able to listen to plugin events you need to implement a methods from callback `ProductDBPluginEventsListener`. You can subscribe to this events by calling meeting.addPluginEventsListener(dytePluginEventsListener).
