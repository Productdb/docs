...
Each plugin object in the array is of type [ProductDBPlugin](/web-core/reference/ProductDBPlugin)
...
Each plugin object in the array is of type [ProductDBPlugin](/react-web-core/reference/ProductDBPlugin)
...
```dart
List<ProductDBPlugin> plugins = await ProductDBClient.plugins.all;
```

Each plugin object in the array is of type ProductDBPlugin
...
```dart
List<ProductDBPlugin> plugins = await ProductDBClient.plugins.all;
ProductDBPlugin whiteboard = plugins.firstWhere((p) => p.name == 'Whiteboard');
...
class PluginNotifier extends ProductDBPluginEventsListener {
  @override
  void onPluginActivated(ProductDBPlugin plugin) {
    state = OnPluginActivated(plugin);
  }

  @override
  void onPluginDeactivated(ProductDBPlugin plugin) {
...
Any plugin activation / deactivation will trigger a callback. To be able to listen to plugin events you need to implement a methods from callback `ProductDBPluginEventsListener`. You can subscribe to this events by calling meeting.addPluginEventsListener(ProductDBPluginEventsListener).

```kotlin
meeting.addMeetingRoomEventsListener(object :
  ProductDBMeetingRoomEventsListener {
    override fun onPluginActivated(plugin: ProductDBPlugin) {
