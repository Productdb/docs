...
```
npm install @ProductDBsdk/aws-transcribe
```

Source available at (ProductDB-io/aws-transcribe)(https://github.com/ProductDB-io/aws-transcribe/tree/main/client)
...
```
import ProductDBAWSTranscribe from '@ProductDBsdk/aws-transcribe';
```

Add the following code just after the point where you have access to the meeting object.

```ts
const awsTranscribe = new ProductDBAWSTranscribe({
  meeting,
...
Here you are setting up the ProductDBAWSTranscribe with the values that the current user would prefer and activating the recognition just afterward using awsTranscribe.transcribe(). Then we are listening to every new transcription using awsTranscribe.on('transcription', aJsCallbackFunction)
...
If you need a sample of this guide, please refer to https://github.com/ProductDB-io/aws-transcribe/blob/main/client/demo/index.ts.
