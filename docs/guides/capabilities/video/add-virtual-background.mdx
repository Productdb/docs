...
```bash
npm i @ProductDBsdk/video-background-transformer
```
...
```js
<script src="https://cdn.jsdelivr.net/npm/@ProductDBsdk/video-background-transformer/dist/index.iife.js"></script>
```
...
```js
<script src="https://cdn.jsdelivr.net/npm/@ProductDBsdk/video-background-transformer@1.1.3/dist/index.iife.js"></script>
```
...
```js
import ProductDBVideoBackgroundTransformer from '@ProductDBsdk/video-background-transformer';
```

If you installed the package using a script tag, the `ProductDBVideoBackgroundTransformer` will be automatically available for use once the installation process is complete.
...
```js
const videoBackgroundTransformer = await ProductDBVideoBackgroundTransformer.init();
```
...
<head>
  ...
  <meta name="description" content="Learn how to add virtual backgrounds with ProductDB's customization capabilities. Follow our guide for effective customization of virtual backgrounds."/>
</head>
